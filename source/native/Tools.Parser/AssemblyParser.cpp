...
HRESULT MetaData::TypeSignature::Parse( CLR_RT_StringVector& sig, CLR_RT_StringVector::size_type& pos )
{
    ...
    while(true)
    {
        ...
        else                                     { NANOCLR_SET_AND_LEAVE(CLR_E_NOTIMPL); }

        switch(m_opt)
        {
            case ELEMENT_TYPE_VOID      : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_BOOLEAN   : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_CHAR      : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_I1        : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_U1        : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_I2        : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_U2        : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_I4        : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_U4        : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_I8        : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_U8        : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_R4        : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_R8        : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_STRING    : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_PTR       : NANOCLR_SET_AND_LEAVE(ParseSubType( sig, pos ));
            case ELEMENT_TYPE_BYREF     : NANOCLR_SET_AND_LEAVE(ParseSubType( sig, pos ));
            case ELEMENT_TYPE_VALUETYPE : NANOCLR_SET_AND_LEAVE(ParseToken  ( sig, pos ));
            case ELEMENT_TYPE_CLASS     : NANOCLR_SET_AND_LEAVE(ParseToken  ( sig, pos ));
            case ELEMENT_TYPE_ARRAY     : NANOCLR_SET_AND_LEAVE(ParseArray  ( sig, pos ));
            case ELEMENT_TYPE_TYPEDBYREF: NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_I         : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_U         : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_FNPTR     : NANOCLR_SET_AND_LEAVE(ParseToken  ( sig, pos ));
            case ELEMENT_TYPE_OBJECT    : NANOCLR_SET_AND_LEAVE(S_OK                    );
            case ELEMENT_TYPE_SZARRAY   : NANOCLR_SET_AND_LEAVE(ParseSzArray( sig, pos ));
        }
    }
...
#define HELPER_QUICKSTRING_CHECK(buf,size) if(buf.MaxSize() > size) { buf[size] = 0; break; } NANOCLR_CHECK_HRESULT(buf.ReSizeNoThrow( size+1 ))
...
HRESULT MetaData::Parser::GetAssemblyDef()
{
    ...
    if(tkAsm != mdAssemblyNil)
    {
        ...
        WCHAR            wzName[1024];
        ULONG            ulNameLen = 0;
        ASSEMBLYMETADATA md; NANOCLR_CLEAR(md);
        WCHAR            wzLocale[1024];
        DWORD            dwFlags;
        ...
        NANOCLR_CHECK_HRESULT(m_pAssemblyImport->GetAssemblyProps(  tkAsm            ,   // [IN ] The Assembly for which to get the properties.
...
HRESULT MetaData::Parser::GetAssemblyRef( mdAssemblyRef ar )
{
    ...
    if(m_mapRef_Assembly.find( ar ) == m_mapRef_Assembly.end())
    {
        ...
        while(true)
        {
            NANOCLR_CHECK_HRESULT(m_pAssemblyImport->GetAssemblyRefProps( ar, NULL, NULL, HELPER_QUICKSTRING_CALL(buf,size), &md, NULL, NULL, &db.m_flags ));
...
HRESULT MetaData::Parser::GetMemberRef( mdMemberRef mr )
{
    ...
    if(m_mapRef_Member.find( mr ) == m_mapRef_Member.end())
    {
        ...
        while(true)
        {
            NANOCLR_CHECK_HRESULT(m_pImport->GetMemberRefProps( mr, &db.m_tr, HELPER_QUICKSTRING_CALL(buf,size), &pSigBlob, &cbSigBlob ));
...
HRESULT MetaData::Parser::GetTypeDef( mdTypeDef td )
{
    ...
    if(m_mapDef_Type.find( td ) == m_mapDef_Type.end())
    {
        ...
        while(true)
        {
            NANOCLR_CHECK_HRESULT(m_pImport->GetTypeDefProps( td, HELPER_QUICKSTRING_CALL(buf,size), &db.m_flags, &db.m_extends ));
...
HRESULT MetaData::Parser::GetTypeField( mdFieldDef fd )
{
    ...
    if(m_mapDef_Field.find( fd ) == m_mapDef_Field.end())
    {
        ...
        while(true)
        {
            NANOCLR_CHECK_HRESULT(m_pImport->GetFieldProps( fd, &db.m_td, HELPER_QUICKSTRING_CALL(buf,size), &db.m_flags, &pSigBlob, &cbSigBlob, &db.m_attr, &pValue, &chValue ));
        ...
        if(IsFdHasFieldRVA( db.m_flags ))
        {
            ...
            if(methRVA)
            {
                ...
                if(m_pe.GetVAforRVA( methRVA, VA ))
                {
                    ...
                    {
                        NANOCLR_CHECK_HRESULT(m_pImport->GetClassLayout( db.m_sig.m_sigField.m_token, NULL, NULL, 0, NULL, &ulSize ));
                    }
                    else
                    {
...
HRESULT MetaData::Parser::GetTypeMethod( mdMethodDef md )
{
    ...
    {        
        ...
        while(true)
        {
            NANOCLR_CHECK_HRESULT(m_pImport->GetMethodProps( md, &db.m_td, HELPER_QUICKSTRING_CALL(buf,size), &db.m_flags, &pSigBlob, &cbSigBlob, &db.m_RVA, &db.m_implFlags ));
...
HRESULT MetaData::Parser::GetManifestResource( mdManifestResource mr )
{
    ...
    if(m_mapDef_ManifestResource.find( mr ) == m_mapDef_ManifestResource.end())
    {
        ...
        while(true)
        {
            NANOCLR_CHECK_HRESULT(m_pAssemblyImport->GetManifestResourceProps( mr, HELPER_QUICKSTRING_CALL(buf,size), &tkImplementation, &dwOffset, &dwResourceFlags ));
...
HRESULT MetaData::Parser::EnumGenericParams( mdToken tk )
{
    ...
    if(!m_pImport2) NANOCLR_SET_AND_LEAVE(S_OK);
...
HRESULT MetaData::Parser::Analyze( LPCWSTR szFileName )
{
    ...
    NANOCLR_CHECK_HRESULT(spRuntimeInfo->GetInterface(CLSID_CorMetaDataDispenser, IID_IMetaDataDispenserEx, (LPVOID*)&m_pDisp));
    ...
    case mdtTypeDef:
        {
            ...
            for(mdFieldDefListIter itFieldDef = td.m_fields.begin(); itFieldDef != td.m_fields.end(); itFieldDef++)
            {
                ...
                NANOCLR_CHECK_HRESULT(CanIncludeMember( tk, tf )); if(hr == S_FALSE) continue;
            ...
            for(mdMethodDefListIter itMethodDef = td.m_methods.begin(); itMethodDef != td.m_methods.end(); itMethodDef++)
            {
                ...
                NANOCLR_CHECK_HRESULT(CanIncludeMember( tk, tm )); if(hr == S_FALSE) continue;
            ...
            for(mdInterfaceImplListIter itInterfaceImpl = td.m_interfaces.begin(); itInterfaceImpl != td.m_interfaces.end(); itInterfaceImpl++)
            {
                ...
                NANOCLR_CHECK_HRESULT(CanIncludeMember( tk, ti )); if(hr == S_FALSE) continue;
    ...
    for(TypeDefMapIter itTypeDef = m_mapDef_Type.begin(); itTypeDef != m_mapDef_Type.end(); itTypeDef++)
    {
        ...
        for(mdFieldDefListIter itFieldDef = td.m_fields.begin(); itFieldDef != td.m_fields.end(); itFieldDef++)
        {
            ...
            else
            {
                ...
                fd.m_sig.ExtractTypeRef( set ); NANOCLR_CHECK_HRESULT(CheckTokensPresence( set ));
            }
        }
        ...
        for(mdMethodDefListIter itMethodDef = td.m_methods.begin(); itMethodDef != td.m_methods.end(); itMethodDef++)
        {
            ...
            else
            {
                ...
                md.m_method.ExtractTypeRef( set ); NANOCLR_CHECK_HRESULT(CheckTokensPresence( set ));

                if(m_fNoByteCode == false)
                {
                    md.m_vars.ExtractTypeRef( set ); NANOCLR_CHECK_HRESULT(CheckTokensPresence( set ));
