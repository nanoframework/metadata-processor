...
template <class T> void RemoveUnusedItems(std::map<mdToken, T> &d, MetaData::mdTokenSet &s)
{
    ...
    typename std::map<mdToken, T>::iterator it;
...
bool MetaData::TypeSignature::operator==(const TypeSignature &sig) const
{
    if (m_opt != sig.m_opt)
        return false;
    if (m_rank != sig.m_rank)
        return false;
    if (m_sizes != sig.m_sizes)
        return false;
    if (m_lowBounds != sig.m_lowBounds)
        return false;
    ...
    if (m_token != mdTokenNil)
    {
        ...
        if (sig.m_token == mdTokenNil)
            return false;

        if (FAILED(m_holder->m_holder->ResolveTypeDef(m_holder, m_token, prLeft, tdLeft)))
            return false;
        if (FAILED(sig.m_holder->m_holder->ResolveTypeDef(sig.m_holder, sig.m_token, prRight, tdRight)))
            return false;
    ...
    while (true)
    {
        ...
        switch (m_opt)
        {
            ...
            case ELEMENT_TYPE_PTR:
                return ParseSubType(pSigBlob);
            case ELEMENT_TYPE_BYREF:
                return ParseSubType(pSigBlob);
            case ELEMENT_TYPE_VALUETYPE:
                return ParseToken(pSigBlob);
            case ELEMENT_TYPE_CLASS:
                return ParseToken(pSigBlob);
            case ELEMENT_TYPE_ARRAY:
                return ParseArray(pSigBlob);
...
HRESULT MetaData::TypeSignature::ParseSubType(PCCOR_SIGNATURE &pSigBlob)
{
    ...
    NANOCLR_SET_AND_LEAVE(m_sub->Parse(pSigBlob));
...
HRESULT MetaData::TypeSignature::ParseSzArray(PCCOR_SIGNATURE &pSigBlob)
{
    ...
    NANOCLR_SET_AND_LEAVE(ParseSubType(pSigBlob));
...
HRESULT MetaData::TypeSignature::ParseArray(PCCOR_SIGNATURE &pSigBlob)
{
    ...
    NANOCLR_CHECK_HRESULT(ParseSubType(pSigBlob));

    m_rank = CorSigUncompressData(pSigBlob);
    numSizes = CorSigUncompressData(pSigBlob);
    while (numSizes-- > 0)
    {
        m_sizes.push_back(CorSigUncompressData(pSigBlob));
    }

    numLoBounds = CorSigUncompressData(pSigBlob);
    while (numLoBounds-- > 0)
    {
...
#define PARSESIGNATURE_CHECKRANGE(sig, pos)                                                                            \
    if (pos >= sig.size())                                                                                             \
    NANOCLR_MSG_SET_AND_LEAVE(CLR_E_FAIL, L"Failed in assembly parser: signature size.\n")
#define PARSESIGNATURE_SCAN(sig, pos, fmt, val)                                                                        \
    if (swscanf_s(sig[pos++].c_str(), fmt, &val) != 1)                                                                 \
    NANOCLR_MSG_SET_AND_LEAVE(CLR_E_FAIL, L"Failed in assembly parser: signature scan.\n")

#define PARSESIGNATURE_CLEANUP(hr, sig)                                                                                \
    NANOCLR_CLEANUP();                                                                                                 \
    if (FAILED(hr) && hr != CLR_E_PARSER_BAD_TEXT_SIGNATURE)                                                           \
    {                                                                                                                  \
        hr = CLR_E_PARSER_BAD_TEXT_SIGNATURE;                                                                          \
        DumpSignature(sig);                                                                                            \
    }                                                                                                                  \
    NANOCLR_CLEANUP_END()
...
HRESULT MetaData::TypeSignature::Parse(CLR_RT_StringVector &sig, CLR_RT_StringVector::size_type &pos)
{
    ...
    while (true)
    {
        ...
        if (!_wcsicmp(val, L"VOID"))
        {
            m_opt = ELEMENT_TYPE_VOID;
        }
        else if (!_wcsicmp(val, L"BOOLEAN"))
        {
            m_opt = ELEMENT_TYPE_BOOLEAN;
        }
        else if (!_wcsicmp(val, L"CHAR"))
        {
            m_opt = ELEMENT_TYPE_CHAR;
        }
        else if (!_wcsicmp(val, L"I1"))
        {
            m_opt = ELEMENT_TYPE_I1;
        }
        else if (!_wcsicmp(val, L"U1"))
        {
            m_opt = ELEMENT_TYPE_U1;
        }
        else if (!_wcsicmp(val, L"I2"))
        {
            m_opt = ELEMENT_TYPE_I2;
        }
        else if (!_wcsicmp(val, L"U2"))
        {
            m_opt = ELEMENT_TYPE_U2;
        }
        else if (!_wcsicmp(val, L"I4"))
        {
            m_opt = ELEMENT_TYPE_I4;
        }
        else if (!_wcsicmp(val, L"U4"))
        {
            m_opt = ELEMENT_TYPE_U4;
        }
        else if (!_wcsicmp(val, L"I8"))
        {
            m_opt = ELEMENT_TYPE_I8;
        }
        else if (!_wcsicmp(val, L"U8"))
        {
            m_opt = ELEMENT_TYPE_U8;
        }
        else if (!_wcsicmp(val, L"R4"))
        {
            m_opt = ELEMENT_TYPE_R4;
        }
        else if (!_wcsicmp(val, L"R8"))
        {
            m_opt = ELEMENT_TYPE_R8;
        }
        else if (!_wcsicmp(val, L"STRING"))
        {
            m_opt = ELEMENT_TYPE_STRING;
        }
        else if (!_wcsicmp(val, L"PTR"))
        {
            m_opt = ELEMENT_TYPE_PTR;
        }
        else if (!_wcsicmp(val, L"BYREF"))
        {
            m_opt = ELEMENT_TYPE_BYREF;
        }
        else if (!_wcsicmp(val, L"VALUETYPE"))
        {
            m_opt = ELEMENT_TYPE_VALUETYPE;
        }
        else if (!_wcsicmp(val, L"CLASS"))
        {
            m_opt = ELEMENT_TYPE_CLASS;
        }
        else if (!_wcsicmp(val, L"ARRAY"))
        {
            m_opt = ELEMENT_TYPE_ARRAY;
        }
        else if (!_wcsicmp(val, L"TYPEDBYREF"))
        {
            m_opt = ELEMENT_TYPE_TYPEDBYREF;
        }
        else if (!_wcsicmp(val, L"I"))
        {
            m_opt = ELEMENT_TYPE_I;
        }
        else if (!_wcsicmp(val, L"U"))
        {
            m_opt = ELEMENT_TYPE_U;
        }
        else if (!_wcsicmp(val, L"FNPTR"))
        {
            m_opt = ELEMENT_TYPE_FNPTR;
        }
        else if (!_wcsicmp(val, L"OBJECT"))
        {
            m_opt = ELEMENT_TYPE_OBJECT;
        }
        else if (!_wcsicmp(val, L"SZARRAY"))
        {
            m_opt = ELEMENT_TYPE_SZARRAY;
        }
        else
        {
        ...
        switch (m_opt)
        {
            ...
            case ELEMENT_TYPE_STRING:
                NANOCLR_SET_AND_LEAVE(S_OK);
            case ELEMENT_TYPE_PTR:
                NANOCLR_SET_AND_LEAVE(ParseSubType(sig, pos));
            case ELEMENT_TYPE_BYREF:
                NANOCLR_SET_AND_LEAVE(ParseSubType(sig, pos));
            case ELEMENT_TYPE_VALUETYPE:
                NANOCLR_SET_AND_LEAVE(ParseToken(sig, pos));
            case ELEMENT_TYPE_CLASS:
                NANOCLR_SET_AND_LEAVE(ParseToken(sig, pos));
            case ELEMENT_TYPE_ARRAY:
                NANOCLR_SET_AND_LEAVE(ParseArray(sig, pos));
            case ELEMENT_TYPE_TYPEDBYREF:
                NANOCLR_SET_AND_LEAVE(S_OK);
            case ELEMENT_TYPE_I:
                NANOCLR_SET_AND_LEAVE(S_OK);
            ...
            case ELEMENT_TYPE_OBJECT:
                NANOCLR_SET_AND_LEAVE(S_OK);
            case ELEMENT_TYPE_SZARRAY:
                NANOCLR_SET_AND_LEAVE(ParseSzArray(sig, pos));
        }
    }
...
HRESULT
MetaData::TypeSignature::ParseToken(CLR_RT_StringVector &sig, CLR_RT_StringVector::size_type &pos)
{
    NANOCLR_HEADER();
...
HRESULT
MetaData::TypeSignature::ParseSubType(CLR_RT_StringVector &sig, CLR_RT_StringVector::size_type &pos)
{
    NANOCLR_HEADER();

    m_sub = new TypeSignature(m_holder);

    NANOCLR_SET_AND_LEAVE(m_sub->Parse(sig, pos));

    PARSESIGNATURE_CLEANUP(hr, sig);
}

HRESULT
MetaData::TypeSignature::ParseSzArray(CLR_RT_StringVector &sig, CLR_RT_StringVector::size_type &pos)
{
    NANOCLR_HEADER();
...
HRESULT
MetaData::TypeSignature::ParseArray(CLR_RT_StringVector &sig, CLR_RT_StringVector::size_type &pos)
{
    NANOCLR_HEADER();
    ...
    NANOCLR_CHECK_HRESULT(ParseSubType(sig, pos));
...
HRESULT MetaData::MethodSignature::Parse(PCCOR_SIGNATURE &pSigBlob)
{
    ...
    {
        NANOCLR_SET_AND_LEAVE(CLR_E_PARSER_UNSUPPORTED_GENERICS);
    }

    count = CorSigUncompressData(pSigBlob);

    NANOCLR_CHECK_HRESULT(m_retValue.Parse(pSigBlob));

    for (ULONG i = 0; i < count; i++)
    {
        TypeSignature param(m_holder);

        NANOCLR_CHECK_HRESULT(param.Parse(pSigBlob));
...
HRESULT MetaData::MethodSignature::Parse(CLR_RT_StringVector &sig, CLR_RT_StringVector::size_type &pos)
{
    ...
    NANOCLR_CHECK_HRESULT(m_retValue.Parse(sig, pos));

    for (ULONG i = 0; i < count; i++)
    {
        TypeSignature param(m_holder);

        NANOCLR_CHECK_HRESULT(param.Parse(sig, pos));
...
{
    if (m_flags != sig.m_flags)
        return false;
    if (m_retValue != sig.m_retValue)
        return false;
    if (m_lstParams != sig.m_lstParams)
        return false;
...
HRESULT MetaData::LocalVarSignature::Parse(PCCOR_SIGNATURE &pSigBlob)
{
    ...
    if (*pSigBlob++ != IMAGE_CEE_CS_CALLCONV_LOCAL_SIG)
        NANOCLR_MSG_SET_AND_LEAVE(CLR_E_FAIL, L"Failed in assembly parser: parsing signature blob.\n");
    ...
    for (ULONG i = 0; i < count; i++)
    {
        ...
        NANOCLR_CHECK_HRESULT(var.Parse(pSigBlob));
...
HRESULT
MetaData::LocalVarSignature::Parse(CLR_RT_StringVector &sig, CLR_RT_StringVector::size_type &pos)
{
    NANOCLR_HEADER();
    ...
    if (val != IMAGE_CEE_CS_CALLCONV_LOCAL_SIG)
        NANOCLR_MSG_SET_AND_LEAVE(CLR_E_FAIL, L"Failed in assembly parser: parsing signature position in blob.\n");
    ...
    for (ULONG i = 0; i < count; i++)
    {
        ...
        NANOCLR_CHECK_HRESULT(var.Parse(sig, pos));
...
MetaData::TypeSpecSignature::TypeSpecSignature(Parser *holder)
    : m_sigField(holder), m_sigLocal(holder), m_sigMethod(holder)
{
    m_holder = holder; // Parser*           m_holder;
    //
...
//
// MetaData::TypeSpecSignature::TypeSpecSignature( const TypeSpecSignature& tss
// ) : m_sigField(tss.m_sigField), m_sigLocal(tss.m_sigLocal),
// m_sigMethod(tss.m_sigMethod)
//{
...
//}
//
// MetaData::TypeSpecSignature& MetaData::TypeSpecSignature::operator= ( const
// TypeSpecSignature& tss )
//{
//    m_holder    = tss.m_holder;    // Parser*           m_holder;
//                                   //
//    m_type      = tss.m_type;      // mdToken           m_type;
...
void MetaData::TypeSpecSignature::ExtractTypeRef(mdTokenSet &set)
{
    ...
    {
        case mdtFieldDef:
            m_sigField.ExtractTypeRef(set);
            break;
        case mdtSignature:
            m_sigLocal.ExtractTypeRef(set);
            break;
        case mdtMethodDef:
            m_sigMethod.ExtractTypeRef(set);
            break;
    }
}
...
HRESULT MetaData::TypeSpecSignature::Parse(PCCOR_SIGNATURE &pSigBlob)
{
    ...
    switch (*pSigBlob & IMAGE_CEE_CS_CALLCONV_MASK)
    {
        case IMAGE_CEE_CS_CALLCONV_FIELD:
            NANOCLR_CHECK_HRESULT(m_sigField.Parse(++pSigBlob));

            m_type = mdtFieldDef;
            break;

        case IMAGE_CEE_CS_CALLCONV_LOCAL_SIG:
            NANOCLR_CHECK_HRESULT(m_sigLocal.Parse(pSigBlob));

            m_type = mdtSignature;
            break;

        case IMAGE_CEE_CS_CALLCONV_DEFAULT: // A method.
            NANOCLR_CHECK_HRESULT(m_sigMethod.Parse(pSigBlob));
...
HRESULT
MetaData::TypeSpecSignature::Parse(CLR_RT_StringVector &sig, CLR_RT_StringVector::size_type &pos)
{
    NANOCLR_HEADER();
    ...
    {
        case IMAGE_CEE_CS_CALLCONV_FIELD:
            NANOCLR_CHECK_HRESULT(m_sigField.Parse(sig, pos));

            m_type = mdtFieldDef;
            break;

        case IMAGE_CEE_CS_CALLCONV_LOCAL_SIG:
            NANOCLR_CHECK_HRESULT(m_sigLocal.Parse(sig, pos));

            m_type = mdtSignature;
            break;

        case IMAGE_CEE_CS_CALLCONV_DEFAULT: // A method.
            NANOCLR_CHECK_HRESULT(m_sigMethod.Parse(sig, pos));
...
MetaData::AssemblyRef::AssemblyRef()
{
    ...
    // std::wstring       m_name;
    NANOCLR_CLEAR(m_version); // CLR_RECORD_VERSION m_version;
}
...
{
    if (m_pos + size > &m_blob[0] + m_blob.size())
        return false;
...
MetaData::CustomAttribute::Writer::Writer(WatchAssemblyBuilder::Linker *lk, BYTE *&ptr, BYTE *end)
    : m_lk(lk), m_ptr(ptr), m_end(end)
{
}
...
bool MetaData::CustomAttribute::Writer::Write(const std::wstring &val)
{
    ...
    if (!m_lk->AllocString(val, idx, false))
        return false;
...
bool MetaData::CustomAttribute::Value::Parse(Reader &reader)
{
    switch (m_opt)
    {
        case SERIALIZATION_TYPE_BOOLEAN:
            return reader.Read(m_numeric.u1);
        case SERIALIZATION_TYPE_CHAR:
            return reader.Read(m_numeric.u2);
        ...
        case SERIALIZATION_TYPE_I4:
            return reader.Read(m_numeric.u4);
        case SERIALIZATION_TYPE_U4:
            return reader.Read(m_numeric.u4);
        case SERIALIZATION_TYPE_I8:
            return reader.Read(m_numeric.u8);
...
{
    if (writer.Write((CLR_UINT8)m_opt) == false)
        return false;

    switch (m_opt)
    {
        case SERIALIZATION_TYPE_BOOLEAN:
            return writer.Write(m_numeric.u1);
        case SERIALIZATION_TYPE_CHAR:
            return writer.Write(m_numeric.u2);
        case SERIALIZATION_TYPE_I1:
            return writer.Write(m_numeric.u1);
        case SERIALIZATION_TYPE_U1:
            return writer.Write(m_numeric.u1);
        ...
        case SERIALIZATION_TYPE_U2:
            return writer.Write(m_numeric.u2);
        case SERIALIZATION_TYPE_I4:
            return writer.Write(m_numeric.u4);
        case SERIALIZATION_TYPE_U4:
            return writer.Write(m_numeric.u4);
        case SERIALIZATION_TYPE_I8:
            return writer.Write(m_numeric.u8);
        case SERIALIZATION_TYPE_U8:
            return writer.Write(m_numeric.u8);
        case SERIALIZATION_TYPE_R4:
            return writer.Write(m_numeric.u4);
...
HRESULT MetaData::CustomAttribute::Parse()
{
    ...
    NANOCLR_CHECK_HRESULT(m_holder->m_holder->ResolveMethodDef(m_holder, m_tkType, prDst, mdDst));

    {
        Parser *prDst2;
        TypeDef *tdDst2;

        NANOCLR_CHECK_HRESULT(m_holder->m_holder->ResolveTypeDef(prDst, mdDst->m_td, prDst2, tdDst2));
...

    {
        ...
        for (TypeSignatureIter itTS = mdDst->m_method.m_lstParams.begin(); itTS != mdDst->m_method.m_lstParams.end();
             itTS++)
        {
            ...
            if (sig.m_opt == ELEMENT_TYPE_VALUETYPE)
            {
                ...
                NANOCLR_CHECK_HRESULT(m_holder->m_holder->ResolveTypeDef(sig.m_holder, sig.m_token, prDst2, tdDst2));
        ...
        while (val)
        {
            ...
            if (v.m_opt == SERIALIZATION_TYPE_ENUM)
            {
                ...
                if (reader.Read(enumType) == false)
                    return false;
            }

            if (!reader.Read(n.m_text))
                goto InvalidBlob;
        ...
        if (reader.m_pos != &m_blob[0] + m_blob.size())
            goto InvalidBlob;
...
MetaData::Parser::Parser(Collection *holder)
{
    ...
    // mdTokenSet m_setAttributes_Methods_GloballySynchronized; mdTokenSet
    // m_setAttributes_Types_PublishInApplicationDirectory;
    //
...
#define HELPER_QUICKSTRING_DECL(buf, size)                                                                             \
    CQuickWSTR buf;                                                                                                    \
    ULONG size;                                                                                                        \
    buf.Maximize();

#define HELPER_QUICKSTRING_CALL(buf, size) buf.Ptr(), (ULONG)(buf.MaxSize() - 1), &size
#define HELPER_QUICKSTRING_CHECK(buf, size)                                                                            \
    if (buf.MaxSize() > size)                                                                                          \
    {                                                                                                                  \
        buf[size] = 0;                                                                                                 \
        break;                                                                                                         \
    }                                                                                                                  \
    NANOCLR_CHECK_HRESULT(buf.ReSizeNoThrow(size + 1))
...
HRESULT MetaData::Parser::GetAssemblyDef()
{
    ...
    NANOCLR_CHECK_HRESULT(m_pAssemblyImport->GetAssemblyFromScope(&tkAsm));
    ...
    if (tkAsm != mdAssemblyNil)
    {
        ...
        NANOCLR_CHECK_HRESULT(m_pAssemblyImport->GetAssemblyProps(
...
HRESULT MetaData::Parser::GetAssemblyRef(mdAssemblyRef ar)
{
    ...
    if (m_mapRef_Assembly.find(ar) == m_mapRef_Assembly.end())
    {
        ...
        while (true)
        {
            NANOCLR_CHECK_HRESULT(m_pAssemblyImport->GetAssemblyRefProps(
                ar,
                NULL,
                NULL,
                HELPER_QUICKSTRING_CALL(buf, size),
                &md,
                NULL,
                NULL,
                &db.m_flags));
...
HRESULT MetaData::Parser::GetModuleRef(mdModuleRef mr)
{
    ...
    if (m_mapRef_Module.find(mr) == m_mapRef_Module.end())
    {
        ...
        while (true)
        {
            NANOCLR_CHECK_HRESULT(m_pImport->GetModuleRefProps(mr, HELPER_QUICKSTRING_CALL(buf, size)));
...
HRESULT MetaData::Parser::GetTypeRef(mdTypeRef tr)
{
    ...
    if (m_mapRef_Type.find(tr) == m_mapRef_Type.end())
    {
        ...
        while (true)
        {
            NANOCLR_CHECK_HRESULT(m_pImport->GetTypeRefProps(tr, &db.m_scope, HELPER_QUICKSTRING_CALL(buf, size)));
            ...
            if (size > MAXTYPENAMELEN)
            {
                ...
                // if(SUCCEEDED(ErrorReporting::ConstructErrorOrigin( str, m_pSymReader,
                // md, 0 )))
                {
                    ErrorReporting::Print(
                        str.c_str(),
                        NULL,
                        TRUE,
                        0,
                        L"Length of name of type '%s' (%d) is longer than %d characters",
                        buf.Ptr(),
                        size,
                        MAXTYPENAMELEN);
                }
        ...
        NANOCLR_CHECK_HRESULT(EnumMemberRefs(db));
    }
    ...
    if (m_mapRef_Member.find(mr) == m_mapRef_Member.end())
    {
        ...
        while (true)
        {
            NANOCLR_CHECK_HRESULT(
                m_pImport->GetMemberRefProps(mr, &db.m_tr, HELPER_QUICKSTRING_CALL(buf, size), &pSigBlob, &cbSigBlob));
        ...
        NANOCLR_CHECK_HRESULT(db.m_sig.Parse(pSigBlob));
...
HRESULT MetaData::Parser::GetTypeDef(mdTypeDef td)
{
    ...
    if (m_mapDef_Type.find(td) == m_mapDef_Type.end())
    {
        ...
        {
            NANOCLR_CHECK_HRESULT(
                m_pImport->GetTypeDefProps(td, HELPER_QUICKSTRING_CALL(buf, size), &db.m_flags, &db.m_extends));
        ...
        {
            NANOCLR_CHECK_HRESULT(m_pImport->GetNestedClassProps(td, &db.m_enclosingClass));
        }
...
HRESULT MetaData::Parser::GetTypeField(mdFieldDef fd)
{
    ...
    if (m_mapDef_Field.find(fd) == m_mapDef_Field.end())
    {
        ...
        while (true)
        {
            NANOCLR_CHECK_HRESULT(m_pImport->GetFieldProps(
                fd,
                &db.m_td,
                HELPER_QUICKSTRING_CALL(buf, size),
                &db.m_flags,
        ...
        if (IsFdHasFieldRVA(db.m_flags))
        {
            ...
            NANOCLR_CHECK_HRESULT(m_pImport->GetRVA(fd, &methRVA, NULL));
            ...
            if (methRVA)
            {
                ...
                if (m_pe.GetVAforRVA(methRVA, VA))
                {
                    ...
                    {
                        NANOCLR_CHECK_HRESULT(
                            m_pImport->GetClassLayout(db.m_sig.m_sigField.m_token, NULL, NULL, 0, NULL, &ulSize));
                    }
                    else
                    {
        ...
        if (pValue)
        {
            // only ASSERT for non strings because it's acceptable to have strings
            // with 0 length
            if (db.m_attr == ELEMENT_TYPE_STRING)
            {
                if (len == 0)
...
HRESULT MetaData::Parser::GetTypeMethod(mdMethodDef md)
{
    ...
    if (m_mapDef_Method.find(md) == m_mapDef_Method.end())
    {
        ...
        while (true)
        {
            NANOCLR_CHECK_HRESULT(m_pImport->GetMethodProps(
                md,
                &db.m_td,
                HELPER_QUICKSTRING_CALL(buf, size),
                &db.m_flags,
        ...
        NANOCLR_CHECK_HRESULT(db.m_method.Parse(pSigBlob));

        if (m_fNoByteCode == false)
        {
            NANOCLR_CHECK_HRESULT(ParseByteCode(db));
        }

        NANOCLR_CHECK_HRESULT(EnumGenericParams(md));
...
HRESULT MetaData::Parser::GetTypeInterface(mdInterfaceImpl ii)
{
    ...
    if (m_mapDef_Interface.find(ii) == m_mapDef_Interface.end())
    {
        ...
        NANOCLR_CHECK_HRESULT(m_pImport->GetInterfaceImplProps(ii, &db.m_td, &db.m_itf));
    }
...
HRESULT MetaData::Parser::GetCustomAttribute(mdCustomAttribute ca)
{
    ...
    if (m_mapDef_CustomAttribute.find(ca) == m_mapDef_CustomAttribute.end())
    {
        ...
        NANOCLR_CHECK_HRESULT(m_pImport->GetCustomAttributeProps(ca, &db.m_tkObj, &db.m_tkType, &pBlob, &cbSize));
        ...
        switch (TypeFromToken(db.m_tkObj))
        {
            ...
            case mdtMethodDef:
                switch (TypeFromToken(db.m_tkType))
                {
                    case mdtMemberRef:
                    case mdtMethodDef:
                        db.m_blob.resize(cbSize);
                        memcpy(&db.m_blob[0], pBlob, cbSize);
...
HRESULT MetaData::Parser::GetTypeSpec(mdTypeSpec ts)
{
    ...
    if (m_mapSpec_Type.find(ts) == m_mapSpec_Type.end())
    {
        ...
        NANOCLR_CHECK_HRESULT(m_pImport->GetTypeSpecFromToken(ts, &pSigBlob, &cbSigBlob));
...
HRESULT MetaData::Parser::GetUserString(mdString s)
{
    ...
    if (m_mapDef_String.find(s) == m_mapDef_String.end())
    {
        ...
        while (true)
        {
            NANOCLR_CHECK_HRESULT(m_pImport->GetUserString(s, HELPER_QUICKSTRING_CALL(buf, size)));
...
HRESULT MetaData::Parser::GetManifestResource(mdManifestResource mr)
{
    ...
    if (m_mapDef_ManifestResource.find(mr) == m_mapDef_ManifestResource.end())
    {
        ...
        while (true)
        {
            NANOCLR_CHECK_HRESULT(m_pAssemblyImport->GetManifestResourceProps(
                mr,
                HELPER_QUICKSTRING_CALL(buf, size),
                &tkImplementation,
                &dwOffset,
        ...
        db.m_blob.resize(dwSize);
        memcpy(&db.m_blob[0], pResource, dwSize);
...
HRESULT MetaData::Parser::EnumAssemblyRefs()
{
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pAssemblyImport->EnumAssemblyRefs(&num, data, ARRAYSIZE(data), &count));
        ...
        for (ULONG i = 0; i < count; i++)
        {
            ...
            NANOCLR_CHECK_HRESULT(GetAssemblyRef(ar));
        }
    }
    ...
    if (num)
        m_pAssemblyImport->CloseEnum(num);
...
HRESULT MetaData::Parser::EnumModuleRefs()
{
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pImport->EnumModuleRefs(&num, data, ARRAYSIZE(data), &count));
        if (hr == S_FALSE || count == 0)
            break;
        ...
        for (ULONG i = 0; i < count; i++)
        {
            ...
            NANOCLR_CHECK_HRESULT(GetModuleRef(mr));
        }
    }
...
HRESULT MetaData::Parser::EnumTypeRefs()
{
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pImport->EnumTypeRefs(&num, data, ARRAYSIZE(data), &count));
        if (hr == S_FALSE || count == 0)
            break;
        ...
        for (ULONG i = 0; i < count; i++)
        {
            ...
            NANOCLR_CHECK_HRESULT(GetTypeRef(tr));
        }
    }
...
HRESULT MetaData::Parser::EnumMemberRefs(TypeRef &tr)
{
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pImport->EnumMemberRefs(&num, tr.m_tr, data, ARRAYSIZE(data), &count));
        if (hr == S_FALSE || count == 0)
            break;
        ...
        for (ULONG i = 0; i < count; i++)
        {
            ...
            NANOCLR_CHECK_HRESULT(GetMemberRef(mr));
...
HRESULT MetaData::Parser::EnumTypeDefs()
{
    ...
    TypeDef tdCopy;
    HCORENUM num = NULL;
    // Buffer for 100 type tokens ( 400 bytes ). One call to EnumTypeDefs reads
    // 100 records.
    mdTypeDef data[100];
    ULONG count;
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pImport->EnumTypeDefs(&num, data, ARRAYSIZE(data), &count));
        if (hr == S_FALSE || count == 0)
            break;
        ...
        for (ULONG i = 0; i < count; i++)
        {
            ...
            NANOCLR_CHECK_HRESULT(GetTypeDef(td));
        }
    }
    ...
    for (TypeDefMapIter itTypeDef = m_mapDef_Type.begin(); itTypeDef != m_mapDef_Type.end(); itTypeDef++)
    {
        ...
        // Developer note: have to exclude the generic types from the list of types
        // to be processed This is because the generic types are not fully supported
        // at this point
        if (td.m_name == L"System.Action`1" || td.m_name == L"System.Action`2" ||
            td.m_name == L"System.EventHandler`1" || td.m_name == L"System.Func`1" || td.m_name == L"System.Func`2" ||
            td.m_name == L"System.Func`3")
        {
            // This will trigger a breakpoint if the condition is met
            //__debugbreak();
            wprintf(L"Skipping type: %s\n", td.m_name.c_str());
        }
        else
        {
            NANOCLR_CHECK_HRESULT(EnumTypeFields(td));
            NANOCLR_CHECK_HRESULT(EnumTypeMethods(td));
            NANOCLR_CHECK_HRESULT(EnumTypeInterfaces(td));
            NANOCLR_CHECK_HRESULT(EnumGenericParams(td.m_td));
        }
    }
...
HRESULT MetaData::Parser::EnumTypeFields(TypeDef &td)
{
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pImport->EnumFields(&num, td.m_td, data, ARRAYSIZE(data), &count));
        if (hr == S_FALSE || count == 0)
            break;
        ...
        for (ULONG i = 0; i < count; i++)
        {
            ...
            NANOCLR_CHECK_HRESULT(GetTypeField(fd));
...
HRESULT MetaData::Parser::EnumTypeMethods(TypeDef &td)
{
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pImport->EnumMethods(&num, td.m_td, data, ARRAYSIZE(data), &count));
        if (hr == S_FALSE || count == 0)
            break;
        ...
        for (ULONG i = 0; i < count; i++)
        {
            ...
            NANOCLR_CHECK_HRESULT(GetTypeMethod(md));
...
HRESULT MetaData::Parser::EnumTypeInterfaces(TypeDef &td)
{
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pImport->EnumInterfaceImpls(&num, td.m_td, data, ARRAYSIZE(data), &count));
        if (hr == S_FALSE || count == 0)
            break;
        ...
        for (ULONG i = 0; i < count; i++)
        {
            ...
            NANOCLR_CHECK_HRESULT(GetTypeInterface(ii));
...
HRESULT MetaData::Parser::EnumCustomAttributes()
{
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pImport->EnumCustomAttributes(&num, 0, 0, data, ARRAYSIZE(data), &count));
        if (hr == S_FALSE || count == 0)
            break;
        ...
        for (ULONG i = 0; i < count; i++)
        {
            ...
            NANOCLR_CHECK_HRESULT(GetCustomAttribute(ca));
        }
    }
...
HRESULT MetaData::Parser::EnumTypeSpecs()
{
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pImport->EnumTypeSpecs(&num, data, ARRAYSIZE(data), &count));
        if (hr == S_FALSE || count == 0)
            break;
        ...
        for (ULONG i = 0; i < count; i++)
        {
            ...
            NANOCLR_CHECK_HRESULT(GetTypeSpec(ts));
        }
    }
...
HRESULT MetaData::Parser::EnumUserStrings()
{
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pImport->EnumUserStrings(&num, data, ARRAYSIZE(data), &count));
        if (hr == S_FALSE || count == 0)
            break;
        ...
        for (ULONG i = 0; i < count; i++)
        {
            ...
            NANOCLR_CHECK_HRESULT(GetUserString(s));
        }
    }
...
HRESULT MetaData::Parser::EnumManifestResources()
{
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pAssemblyImport->EnumManifestResources(&num, data, ARRAYSIZE(data), &count));
        if (hr == S_FALSE || count == 0)
            break;
        ...
        for (ULONG i = 0; i < count; i++)
        {
            ...
            NANOCLR_CHECK_HRESULT(GetManifestResource(mr));
        }
    }
...
HRESULT MetaData::Parser::EnumGenericParams(mdToken tk)
{
    ...
    while (true)
    {
        NANOCLR_CHECK_HRESULT(m_pImport2->EnumGenericParams(&num, tk, data, ARRAYSIZE(data), &count));
        if (hr == S_FALSE || count == 0)
            break;
...
HRESULT MetaData::Parser::ParseByteCode(MethodDef &db)
{
    ...
    if (db.m_VA)
    {
        ...
        if (ls)
        {
            ...
            NANOCLR_CHECK_HRESULT(m_pImport->GetSigFromToken(ls, &pSigBlob, &cbSigBlob));
            ...
            if (FAILED(hr = db.m_vars.Parse(pSigBlob)))
            {
                ...
                if (SUCCEEDED(ErrorReporting::ConstructErrorOrigin(str, m_pSymReader, db.m_md, 0)))
                {
                    ErrorReporting::Print(
                        ...
                        TRUE,
                        0,
                        L"Cannot parse locals signature for '%s'",
                        db.m_name.c_str());
                }
        ...
        if (il.Code)
        {
            ...
            NANOCLR_CHECK_HRESULT(db.m_byteCode.Parse(m_mapDef_Type.find(db.m_td)->second, db, il));
            ...
            for (size_t i = 0; i < db.m_byteCode.m_exceptions.size(); i++)
            {
                ...
                // If not a filter or finally clause, lookup EH class token for filter
                // clause.
                if ((ref.m_Flags != COR_ILEXCEPTION_CLAUSE_FINALLY) && (ref.m_Flags != COR_ILEXCEPTION_CLAUSE_FILTER))
                {
                    if (!IsNilToken(ref.m_ClassToken))
                        NANOCLR_CHECK_HRESULT(m_holder->ResolveTypeDef(this, ref.m_ClassToken, prDst, tdDst));
                }
            }
        }
    }
...
HRESULT MetaData::Parser::Analyze(LPCWSTR szFileName)
{
    ...
    NANOCLR_CHECK_HRESULT(CLRCreateInstance(CLSID_CLRMetaHost, IID_PPV_ARGS(&spMetaHost)));
    NANOCLR_CHECK_HRESULT(spMetaHost->GetRuntime(L"v4.0.30319", IID_PPV_ARGS(&spRuntimeInfo)));
    NANOCLR_CHECK_HRESULT(
        spRuntimeInfo->GetInterface(CLSID_CorMetaDataDispenser, IID_IMetaDataDispenserEx, (LPVOID *)&m_pDisp));

    NANOCLR_CHECK_HRESULT(m_pDisp->OpenScope(szFileName, ofRead, IID_IMetaDataImport, (IUnknown **)&m_pImport));

    NANOCLR_CHECK_HRESULT(m_pImport->QueryInterface(IID_IMetaDataAssemblyImport, (void **)&m_pAssemblyImport));
    ...
    NANOCLR_CHECK_HRESULT(GetAssemblyDef());
    /****************************************/
    NANOCLR_CHECK_HRESULT(EnumAssemblyRefs());
    NANOCLR_CHECK_HRESULT(EnumModuleRefs());
    NANOCLR_CHECK_HRESULT(EnumTypeRefs());
    /****************************************/
    NANOCLR_CHECK_HRESULT(EnumTypeDefs());
    NANOCLR_CHECK_HRESULT(EnumTypeSpecs());
    NANOCLR_CHECK_HRESULT(EnumUserStrings());
    ...
    if (m_fNoAttributes == false)
    {
        ...
        for (CustomAttributeMapIter itCA = m_mapDef_CustomAttribute.begin(); itCA != m_mapDef_CustomAttribute.end();)
        {
            ...
            NANOCLR_CHECK_HRESULT(ca.Parse());
    ...
    if (m_fNoByteCode == false)
    {
        for (MethodDefMapIter itMD = m_mapDef_Method.begin(); itMD != m_mapDef_Method.end(); itMD++)
        {
            ...
            {
                NANOCLR_CHECK_HRESULT(md.m_byteCode.VerifyConsistency(m_mapDef_Type.find(md.m_td)->second, md, this));
            }
        }
    }
...
HRESULT MetaData::Parser::BuildDependencyList(mdToken tk, mdTokenSet &set)
{
    ...
    switch (TypeFromToken(tk))
    {
        case mdtTypeRef:
        {
            NANOCLR_CHECK_HRESULT(GetTypeRef(tk));
        ...
        case mdtMemberRef:
        {
            NANOCLR_CHECK_HRESULT(GetMemberRef(tk));
        ...
        case mdtTypeSpec:
        {
            NANOCLR_CHECK_HRESULT(GetTypeSpec(tk));
        ...
        case mdtTypeDef:
        {
            NANOCLR_CHECK_HRESULT(GetTypeDef(tk));
            ...
            NANOCLR_CHECK_HRESULT(IncludeAttributes(tk, set));
            ...
            for (mdFieldDefListIter itFieldDef = td.m_fields.begin(); itFieldDef != td.m_fields.end(); itFieldDef++)
            {
                ...
                if (fd.m_flags & fdLiteral)
                    continue; // Don't generate constant fields.

                NANOCLR_CHECK_HRESULT(CanIncludeMember(tk, tf));
                if (hr == S_FALSE)
                    continue;
                 ...
                 itMethodDef++)
            {
                ...
                NANOCLR_CHECK_HRESULT(CanIncludeMember(tk, tm));
                if (hr == S_FALSE)
                    continue;
            ...
            for (mdInterfaceImplListIter itInterfaceImpl = td.m_interfaces.begin();
                 itInterfaceImpl != td.m_interfaces.end();
                 itInterfaceImpl++)
            {
                mdToken ti = (mdToken)*itInterfaceImpl;
                InterfaceImpl &ii = m_mapDef_Interface[ti];

                NANOCLR_CHECK_HRESULT(CanIncludeMember(tk, ti));
                if (hr == S_FALSE)
                    continue;
        ...
        case mdtFieldDef:
        {
            NANOCLR_CHECK_HRESULT(GetTypeField(tk));
        ...
        case mdtMethodDef:
        {
            NANOCLR_CHECK_HRESULT(GetTypeMethod(tk));
        ...
        {
            NANOCLR_CHECK_HRESULT(GetTypeInterface(tk));
...
HRESULT MetaData::Parser::IncludeAttributes(mdToken tk, mdTokenSet &set)
{
    ...
    for (CustomAttributeMapIter itCA = m_mapDef_CustomAttribute.begin(); itCA != m_mapDef_CustomAttribute.end(); itCA++)
    {
        ...
        if (ca.m_tkObj == tk)
        {
            ...
            NANOCLR_CHECK_HRESULT(m_holder->ResolveMethodDef(this, ca.m_tkType, prDst, mdDst));
            NANOCLR_CHECK_HRESULT(m_holder->ResolveTypeDef(prDst, mdDst->m_td, prDst, tdDst));
...
HRESULT MetaData::Parser::RemoveUnused()
{
    ...
    while (setNew.size())
    {
        ...
        for (it = setNew.begin(); it != setNew.end(); it++)
        {
            ...
            NANOCLR_CHECK_HRESULT(BuildDependencyList(tk, setTmp));
...
HRESULT MetaData::Parser::VerifyConsistency()
{
        ...
        while (sig)
        {
            if (sig->m_opt == ELEMENT_TYPE_ARRAY)
            {
                ErrorReporting::Print(
                    ...
                    TRUE,
                    0,
                    L"Unsupported multi-dimensional array: 0x%08X ",
                    itTypeSpec->first);

                NANOCLR_SET_AND_LEAVE(CLR_E_PARSER_UNSUPPORTED_MULTIDIM_ARRAY);
            }
        ...
        switch (mr.m_sig.m_type)
        {
            ...
            default:
                ErrorReporting::Print(
                    ...
                    TRUE,
                    0,
                    L"Unsupported member reference: 0x%08X : %s "
                    L"(Class: 0x%08X) (Signature: 0x%08X)\n",
                    itMemberRef->first,
                    mr.m_name.c_str(),
                    mr.m_tr,
                    mr.m_sig.m_type);
                NANOCLR_SET_AND_LEAVE(CLR_E_PARSER_UNKNOWN_MEMBER_REF);
        }
    }
    ...
    for (TypeDefMapIter itTypeDef = m_mapDef_Type.begin(); itTypeDef != m_mapDef_Type.end(); itTypeDef++)
    {
        ...
        NANOCLR_CHECK_HRESULT(CheckTokenPresence(td.m_extends));
        NANOCLR_CHECK_HRESULT(CheckTokenPresence(td.m_enclosingClass));
        ...
        for (mdFieldDefListIter itFieldDef = td.m_fields.begin(); itFieldDef != td.m_fields.end(); itFieldDef++)
        {
            ...
            if (it == m_mapDef_Field.end())
            {
                ErrorReporting::Print(
                    ...
                    TRUE,
                    0,
                    L"Not all fields for type '%s' can be resolved.\n",
                    td.m_name.c_str());
                NANOCLR_CHECK_HRESULT(CLR_E_PARSER_MISSING_FIELD);
            }
            else
            {
                ...
                fd.m_sig.ExtractTypeRef(set);
                NANOCLR_CHECK_HRESULT(CheckTokensPresence(set));
            }
        }
        ...
        for (mdMethodDefListIter itMethodDef = td.m_methods.begin(); itMethodDef != td.m_methods.end(); itMethodDef++)
        {
            ...
            if (it == m_mapDef_Method.end())
            {
                ErrorReporting::Print(
                    ...
                    TRUE,
                    0,
                    L"Not all methods for type '%s' can be resolved.\n",
                    td.m_name.c_str());
                NANOCLR_CHECK_HRESULT(CLR_E_PARSER_MISSING_METHOD);
            }
            else
            {
                ...
                if (m_fNoByteCode == false)
                {
                    ...
                    for (size_t i = 0; i < md.m_byteCode.m_opcodes.size(); i++)
                    {
                        ...
                        if (ref.m_token != mdTokenNil)
                        {
                            NANOCLR_CHECK_HRESULT(CheckTokenPresence(ref.m_token));
                        }
                    }
                }
            }
             ...
             itInterfaceImpl++)
        {
            ...
            if (it == m_mapDef_Interface.end())
            {
                ErrorReporting::Print(
                    ...
                    TRUE,
                    0,
                    L"Not all interfaces for type '%s' can be resolved.\n",
                    td.m_name.c_str());
                NANOCLR_CHECK_HRESULT(CLR_E_PARSER_MISSING_INTERFACE);
            }
            else
            {
                InterfaceImpl &ii = it->second;

                NANOCLR_CHECK_HRESULT(CheckTokenPresence(ii.m_itf));
            }
        }
    }
    ...
    for (CustomAttributeMapIter itCA = m_mapDef_CustomAttribute.begin(); itCA != m_mapDef_CustomAttribute.end();)
    {
        ...
        NANOCLR_CHECK_HRESULT(CheckTokenPresence(ca.m_tkObj));
        NANOCLR_CHECK_HRESULT(CheckTokenPresence(ca.m_tkType));
    }
...
bool MetaData::Parser::CheckIsTokenPresent(mdToken tk)
{
    ...
    if (fPresent == false)
    {
        ...
        {
            case mdtAssemblyRef:
                fPresent = IsTokenPresent(m_mapRef_Assembly, tk);
                break;
            case mdtModuleRef:
                fPresent = IsTokenPresent(m_mapRef_Module, tk);
                break;
            case mdtTypeRef:
                fPresent = IsTokenPresent(m_mapRef_Type, tk);
                break;
            case mdtMemberRef:
                fPresent = IsTokenPresent(m_mapRef_Member, tk);
                break;

            case mdtTypeDef:
                fPresent = IsTokenPresent(m_mapDef_Type, tk);
                break;
            case mdtFieldDef:
                fPresent = IsTokenPresent(m_mapDef_Field, tk);
                break;
            case mdtMethodDef:
                fPresent = IsTokenPresent(m_mapDef_Method, tk);
                break;
            case mdtInterfaceImpl:
                fPresent = IsTokenPresent(m_mapDef_Interface, tk);
                break;

            case mdtTypeSpec:
                fPresent = IsTokenPresent(m_mapSpec_Type, tk);
                break;

            case mdtString:
                fPresent = IsTokenPresent(m_mapDef_String, tk);
                break;
        }
    }
...
HRESULT MetaData::Parser::CheckTokensPresence(mdTokenSet &set)
{
    ...
    {
        NANOCLR_CHECK_HRESULT(CheckTokenPresence((mdToken)*it));
    }
...
bool MetaData::Collection::FileExists(
...
HRESULT MetaData::Collection::ResolveAssemblyDef(Parser *pr, mdToken tk, Parser *&prDst)
{
    ...
    switch (TypeFromToken(tk))
    {
        case mdtAssemblyRef:
        {
            ...
            if (itAR != pr->m_mapRef_Assembly.end())
            {
                ...
                {
                    ErrorReporting::Print(
                        pr->m_assemblyFile.c_str(),
                        NULL,
                        TRUE,
                        0,
                        L"Cannot resolve assembly reference, '%s' is marked as 'ignore'",
                        name.c_str());
                    NANOCLR_MSG_SET_AND_LEAVE(
                        CLR_E_FAIL,
                        L"Cannot resolve assembly reference, is marked as 'ignore'\n");
                }
                ...
                NANOCLR_CHECK_HRESULT(CreateDependentAssembly(file.c_str(), prDst));
    ...
    ErrorReporting::Print(
        pr->m_assemblyName.c_str(),
        NULL,
        TRUE,
        0,
        L"Cannot resolve assembly %s::%08x!",
        pr->m_assemblyName.c_str(),
        tk);
...
HRESULT MetaData::Collection::ResolveTypeDef(Parser *pr, mdToken tk, Parser *&prDst, TypeDef *&tdDst)
{
        ...
        case mdtTypeRef:
        {
            ...
            if (itTR != pr->m_mapRef_Type.end())
            {
                ...
                switch (TypeFromToken(tr.m_scope))
                {
                    case mdtTypeRef:
                    {
                        NANOCLR_CHECK_HRESULT(ResolveTypeDef(pr, tr.m_scope, prDst, tdDst));

                        for (TypeDefMapIter itTD = prDst->m_mapDef_Type.begin(); itTD != prDst->m_mapDef_Type.end();
                             itTD++)
                        {
                    ...
                    case mdtAssemblyRef:
                    {
                        NANOCLR_CHECK_HRESULT(ResolveAssemblyDef(pr, tr.m_scope, prDst));
    ...
    {
        WCHAR rgBuffer[64];
        swprintf(rgBuffer, ARRAYSIZE(rgBuffer), L"%08x", tk);
    ...
    ErrorReporting::Print(
        ...
        TRUE,
        0,
        L"Cannot resolve type %s from assembly %s!\n",
        missing.c_str(),
        pr->m_assemblyName.c_str());
...
HRESULT MetaData::Collection::ResolveMethodDef(Parser *pr, mdToken tk, Parser *&prDst, MethodDef *&mdDst)
{
            ...
            if (itMR != pr->m_mapRef_Member.end())
            {
                ...
                if (mr.m_sig.m_type == mdtMethodDef)
                {
                    ...
                    while (true)
                    {
                        ...
                        NANOCLR_CHECK_HRESULT(ResolveTypeDef(pr, tk, prDst, tdDst));

                        for (mdMethodDefListIter itMDL = tdDst->m_methods.begin(); itMDL != tdDst->m_methods.end();
                             itMDL++)
                        {
    ...
    ErrorReporting::Print(
        ...
        TRUE,
        0,
        L"Cannot resolve method %s from assembly %s!\n",
        missing.c_str(),
        pr->m_assemblyName.c_str());
        ...
        case mdtMemberRef:
        {
            ...
            if (itMR != pr->m_mapRef_Member.end())
            {
                ...
                if (mr.m_sig.m_type == mdtFieldDef)
                {
                    ...
                    NANOCLR_CHECK_HRESULT(ResolveTypeDef(pr, mr.m_tr, prDst, tdDst));
