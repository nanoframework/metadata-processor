...
#define REPORT_NO_MEMORY() { if(IsDebuggerPresent()) { DebugBreak(); } NANOCLR_SET_AND_LEAVE(CLR_E_OUT_OF_MEMORY); }
    ...
    else if(md.m_byteCode.m_opcodes.size() > 0)
    {
        ...
        if(numExceptions > 0)
        {
            ...
            for(size_t i=0; i<numExceptions; i++)
            {
                ...
                CLR_RECORD_EH eh; NANOCLR_CLEAR(eh);
...
HRESULT WatchAssemblyBuilder::Linker::ProcessResource()
{
    ...
    {                    
        ...
        if(!AllocString(strName, resourceFile->name, true)) NANOCLR_SET_AND_LEAVE( CLR_E_OUT_OF_MEMORY );
...
HRESULT WatchAssemblyBuilder::Linker::ResolveTypeDef( MetaData::TypeDef& td, MetaData::mdTypeDefList& order, MetaData::mdTokenSet& resolved )
{
    ...
    if(IsNilToken(td.m_enclosingClass) == false) // Nested class.
    {
        ...
        if(TypeFromToken( td.m_enclosingClass ) == mdtTypeDef)
        {
            ...
            {
                NANOCLR_CHECK_HRESULT(ResolveTypeDef( m_pr->m_mapDef_Type.find(td.m_enclosingClass)->second, order, resolved ));
            }
        }
    }
...
HRESULT WatchAssemblyBuilder::Linker::EmitData( CQuickRecord<BYTE>& buf, CLR_RECORD_ASSEMBLY& headerSrc )
{
    ...
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableAssemblyRef  , buf, headerSrc, TBL_AssemblyRef   , CLR_MaxStreamSize_AssemblyRef   , L"AssemblyRef"    ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableTypeRef      , buf, headerSrc, TBL_TypeRef       , CLR_MaxStreamSize_TypeRef       , L"TypeRef"        ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableFieldRef     , buf, headerSrc, TBL_FieldRef      , CLR_MaxStreamSize_FieldRef      , L"FieldRef"       ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableMethodRef    , buf, headerSrc, TBL_MethodRef     , CLR_MaxStreamSize_MethodRef     , L"MethodRef"      ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableTypeDef      , buf, headerSrc, TBL_TypeDef       , CLR_MaxStreamSize_TypeDef       , L"TypeDef"        ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableFieldDef     , buf, headerSrc, TBL_FieldDef      , CLR_MaxStreamSize_FieldDef      , L"FieldDef"       ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableMethodDef    , buf, headerSrc, TBL_MethodDef     , CLR_MaxStreamSize_MethodDef     , L"MethodDef"      ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableAttribute    , buf, headerSrc, TBL_Attributes    , CLR_MaxStreamSize_Attributes    , L"Attributes"     ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableTypeSpec     , buf, headerSrc, TBL_TypeSpec      , CLR_MaxStreamSize_TypeSpec      , L"TypeSpec"       ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableResource     , buf, headerSrc, TBL_Resources     , CLR_MaxStreamSize_Resources     , L"Resources"      ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableResourceData , buf, headerSrc, TBL_ResourcesData , CLR_MaxStreamSize_ResourcesData , L"ResourcesData"  ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableString       , buf, headerSrc, TBL_Strings       , CLR_MaxStreamSize_Strings       , L"Strings"        ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableSignature    , buf, headerSrc, TBL_Signatures    , CLR_MaxStreamSize_Signatures    , L"Signatures"     ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableByteCode     , buf, headerSrc, TBL_ByteCode      , CLR_MaxStreamSize_ByteCode      , L"ByteCode"       ));
    NANOCLR_CHECK_HRESULT(AppendToAssembly( m_tableResourceFile , buf, headerSrc, TBL_ResourcesFiles, CLR_MaxStreamSize_ResourcesFiles, L"ResourcesFiles" ));
...
HRESULT WatchAssemblyBuilder::Linker::Generate( CQuickRecord<BYTE>& buf, bool patch_fReboot, std::wstring* patch_szNative )
{
    NANOCLR_HEADER();

    CLR_RECORD_ASSEMBLY header; NANOCLR_CLEAR(header);
