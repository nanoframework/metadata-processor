...
#define REPORT_NO_MEMORY() NANOCLR_SET_AND_LEAVE(CLR_E_OUT_OF_MEMORY)
...
void FindRange(CLR_UINT8 v, CLR_INT64 &minValue, CLR_INT64 &maxValue)
{
    minValue = 0;
    maxValue = 255;
}
...
template <typename T, typename V> HRESULT CheckRange(std::list<T> &lst, V field, LPCWSTR szItem, LPCWSTR szKind)
{
    ...
    NANOCLR_SET_AND_LEAVE(CheckRange(lst.size(), field, szItem, szKind));
...
static HRESULT GetFullPath(const std::wstring &szRelative, std::wstring *pAbsolute, std::wstring *pFileName)
{
    ...
    if (cch == 0 || cch >= ARRAYSIZE(buffer))
        NANOCLR_MSG1_SET_AND_LEAVE(CLR_E_FAIL, L"Linker error getting path: %s\n", szRelative);
...
void WatchAssemblyBuilder::ChangeExtensionOnFileName(
    std::wstring strFile,
    std::wstring &strFileNew,
    const wchar_t *szExt)
{
...
WatchAssemblyBuilder::Linker::ExceptionHandlerHierarchy &WatchAssemblyBuilder::Linker::ExceptionHandlerHierarchy::
    operator=(const ExceptionHandlerHierarchy &ehh)
{
    Clear();

    if (ehh.m_data)
    {
        m_data = new CLR_RECORD_EH();
        *m_data = *ehh.m_data;
    }

    for (std::list<ExceptionHandlerHierarchy *>::const_iterator it = ehh.m_children.begin(); it != ehh.m_children.end();
         it++)
    {
...
HRESULT WatchAssemblyBuilder::Linker::ExceptionHandlerHierarchy::GenerateOutput(CQuickRecord<CLR_RECORD_EH> &tbl)
{
    ...
    for (std::list<ExceptionHandlerHierarchy *>::iterator it = m_children.begin(); it != m_children.end(); it++)
    {
        ...
        NANOCLR_CHECK_HRESULT(ehh->GenerateOutput(tbl));
    }

    if (m_data)
    {
        // wprintf( L"%04x %04x %04x %04x\n", m_data->classToken, m_data->tryStart,
        // m_data->tryEnd, m_data->handlerStart );

        CLR_RECORD_EH *dstEH = tbl.Alloc(1);
        if (dstEH == NULL)
            REPORT_NO_MEMORY();
...
bool WatchAssemblyBuilder::Linker::AllocString(const std::string &str, CLR_STRING &idx, bool fUser)
{
    if (fUser == false)
    {
        ...
        if ((str.find("$$method0x") != 0) && (str.find("<PrivateImplementationDetails>{") != 0) &&
    ...
    else
    {
        ...
        CHAR *buf = m_tableString.Alloc(bufSize);
        if (buf == NULL)
            REPORT_FAILURE();
...
bool WatchAssemblyBuilder::Linker::AllocString(const std::wstring &str, CLR_STRING &name, CLR_STRING &nameSpace)
{
    ...
    if (i != str.npos)
    {
        if (!AllocString(str.substr(0, i), nameSpace, false))
            return false;
        if (!AllocString(str.substr(i + 1), name, false))
            return false;
    }
    else
    {
...
bool WatchAssemblyBuilder::Linker::AllocString(
    ...
    CLR_STRING &nameSpace)
{
    ...
    {
        if (!AllocString(strNameSpace, nameSpace, false))
            return false;
        if (!AllocString(strName, name, false))
            return false;
    }
    else
    {
...
HRESULT
WatchAssemblyBuilder::Linker::SaveUniqueStrings(const std::wstring &file)
{
    NANOCLR_HEADER();
    ...
    if (m_collectUniqueStrings.size() > 0)
    {
        for (int pass = 0; pass < 2; pass++)
        {
            ...
            for (std::set<std::string>::iterator it = m_collectUniqueStrings.begin();
                 it != m_collectUniqueStrings.end();
                 it++)
            {
                std::string str = *it;
    ...
    NANOCLR_CHECK_HRESULT(CLR_RT_FileStore::SaveFile(file.c_str(), buffer));

    NANOCLR_NOCLEANUP();
}

HRESULT
WatchAssemblyBuilder::Linker::LoadUniqueStrings(const std::wstring &file)
{
    NANOCLR_HEADER();

    CLR_RT_Buffer buffer;

    NANOCLR_CHECK_HRESULT(CLR_RT_FileStore::LoadFile(file.c_str(), buffer));
    ...
    if (buffer.size() > 0)
    {
        ...
        if (ptrEnd[0] == 0) // It has to be zero terminated!!!
        {
            while (ptr <= ptrEnd)
            {
                m_collectUniqueStrings.insert(std::string((LPCSTR)ptr));
                ptr += hal_strlen_s((LPCSTR)ptr) + 1;
            }
        }
    }
...
HRESULT
WatchAssemblyBuilder::Linker::DumpUniqueStrings(const std::wstring &file)
{
    NANOCLR_HEADER();
...
bool WatchAssemblyBuilder::Linker::CheckDuplicateOrAppend(
    CLR_IDX &idx,
    BYTE *dst,
    size_t dstLen,
    const BYTE *src,
    ...
    while (dstLen >= srcLen)
    {
        if (*dst == *src)
        {
            if (memcmp(dst, src, srcLen) == 0)
            {
                ...
                // dstLenOrig, dst, dstOrig ); CLR_Debug::DumpBufferV( src, srcLen,
                // false, true, NULL, printf );
    ...
    *m_tmpSigPtr++ = MapElementTypeToDataType(sig->m_opt);
    ...
    switch (sig->m_opt)
    {
        ...
        case ELEMENT_TYPE_BYREF:
            return GenerateSignature(sig->m_sub);
        case ELEMENT_TYPE_VALUETYPE:
            return GenerateSignatureToken(sig->m_token);
...
bool WatchAssemblyBuilder::Linker::GenerateSignature(MetaData::CustomAttribute &ca)
{
    ...
    if (!writer.Write((CLR_UINT16)ca.m_valuesVariable.size()))
        REPORT_FAILURE();

    for (MetaData::CustomAttribute::ValueMapIter itV = ca.m_valuesVariable.begin(); itV != ca.m_valuesVariable.end();
         itV++)
    {
        const MetaData::CustomAttribute::Name &n = itV->first;
        MetaData::CustomAttribute::Value &v = itV->second;

        if (!writer.Write(n.m_fField ? SERIALIZATION_TYPE_FIELD : SERIALIZATION_TYPE_PROPERTY))
            REPORT_FAILURE();
        if (!writer.Write(n.m_text))
            REPORT_FAILURE();
...
{
    if (!CLR_CompressData(val, m_tmpSigPtr))
        return false;
...
{
    if (MetaData::IsTokenPresent(m_lookupIDs, tk) == false)
        REPORT_FAILURE();
...
{
    if (CheckDuplicateOrAppend(
            idx,
            (BYTE *)m_tableSignature.Ptr(),
    ...
    BYTE *res = m_tableSignature.Alloc(len);
    if (res == NULL)
        REPORT_FAILURE();
    memcpy(res, ptr, len);
...
bool WatchAssemblyBuilder::Linker::AllocSignatureForMethod(MetaData::MethodSignature *sig, CLR_SIG &idx)
{
    ...
    *m_tmpSigPtr++ = sig->m_flags;
    ...
    if (!GenerateSignatureData8((CLR_UINT8)sig->m_lstParams.size()))
        return false;

    if (!GenerateSignature(&sig->m_retValue))
        return false;

    for (MetaData::TypeSignatureIter it = sig->m_lstParams.begin(); it != sig->m_lstParams.end(); it++)
    {
        if (!GenerateSignature(&(*it)))
            return false;
    }
...
bool WatchAssemblyBuilder::Linker::AllocSignatureForAttribute(MetaData::CustomAttribute &ca, CLR_SIG &idx)
{
    ...
    if (!GenerateSignature(ca))
        return false;

    // CLR_Debug::DumpBufferV( m_tmpSig, (int)(m_tmpSigPtr - m_tmpSig), false,
    // true, NULL, printf );
...
bool WatchAssemblyBuilder::Linker::AllocSignatureForInterfaces(MetaData::mdInterfaceImplList &sig, CLR_SIG &idx)
{
    ...
    if (!GenerateSignatureData8((CLR_UINT8)sig.size()))
        return false;
    ...
    for (MetaData::mdInterfaceImplListIter it = sig.begin(); it != sig.end(); it++)
    {
        ...
        if (!GenerateSignatureToken(ii.m_itf))
            return false;
    }
...
HRESULT WatchAssemblyBuilder::Linker::ValidateTypeRefOrDef(mdToken tk, bool fAsInterface)
{
    ...
    if (IsNilToken(tk))
    {
        if (fAsInterface == false)
            wprintf(L"Linker error validating type ref or def: only interfaces don't "
                    L"extend anything\n");
    ...
    NANOCLR_SET_AND_LEAVE(MetaData::IsTokenPresent(m_lookupIDs, tk) ? S_OK : S_FALSE);
...
HRESULT WatchAssemblyBuilder::Linker::Process(MetaData::Parser &pr)
{
    ...
    for (MetaData::CustomAttributeMapIter itCA = m_pr->m_mapDef_CustomAttribute.begin();
         itCA != m_pr->m_mapDef_CustomAttribute.end();
         itCA++)
    {
        ...
        {
            case mdtTypeDef:
                m_setAttributes_Types.insert(ca.m_tkObj);
                break;
            case mdtFieldDef:
                m_setAttributes_Fields.insert(ca.m_tkObj);
                break;
            case mdtMethodDef:
                m_setAttributes_Methods.insert(ca.m_tkObj);
                break;
        }
    }
    ...
    //
    {
        ...
        if (!AllocString(std::wstring(), idx, true))
            REPORT_NO_MEMORY();
    }
...

    {
        ...
        NANOCLR_CHECK_HRESULT(ProcessAssemblyRef());
        NANOCLR_CHECK_HRESULT(ProcessTypeRef());
        NANOCLR_CHECK_HRESULT(ProcessMemberRef());
        NANOCLR_CHECK_HRESULT(ProcessTypeDef(order));
        NANOCLR_CHECK_HRESULT(ProcessTypeSpec());
        NANOCLR_CHECK_HRESULT(ProcessAttribute());
        NANOCLR_CHECK_HRESULT(ProcessResource());
        NANOCLR_CHECK_HRESULT(ProcessUserString());

        for (MetaData::mdTypeDefListIter itOrder = order.begin(); itOrder != order.end(); itOrder++)
        {
            NANOCLR_CHECK_HRESULT(ProcessTypeDef_ByteCode((mdTypeDef)*itOrder));
        }
    }
...
HRESULT WatchAssemblyBuilder::Linker::ProcessAssemblyRef()
{
    ...
    for (MetaData::AssemblyRefMapIter it = m_pr->m_mapRef_Assembly.begin(); it != m_pr->m_mapRef_Assembly.end(); it++)
    {
        ...
        CLR_RECORD_ASSEMBLYREF *dst = m_tableAssemblyRef.Alloc(1);
        if (dst == NULL)
            REPORT_NO_MEMORY();

        if (!AllocString(ar.m_name, dst->name, false))
            REPORT_NO_MEMORY();
...
HRESULT WatchAssemblyBuilder::Linker::ProcessTypeRef()
{
    ...
    //
    // This is good, because the layout will be ordered and no multiple passes
    // will be required on the runtime end.
    //
    for (it = m_pr->m_mapRef_Type.begin(); it != m_pr->m_mapRef_Type.end(); it++)
    {
        NANOCLR_CHECK_HRESULT(ResolveTypeRef(it->second, order, resolved));
    }
    ...
    for (itOrder = order.begin(); itOrder != order.end(); itOrder++)
    {
        ...
        CLR_RECORD_TYPEREF *dst = m_tableTypeRef.Alloc(1);
        if (dst == NULL)
            REPORT_NO_MEMORY();
    }
    ...
    for (itOrder = order.begin(); itOrder != order.end(); itOrder++)
    {
        ...
        if (!AllocString(tr.m_name, dst->name, dst->nameSpace))
            REPORT_NO_MEMORY();
...
HRESULT WatchAssemblyBuilder::Linker::ProcessMemberRef()
{
    ...
    for (it = m_pr->m_mapRef_Member.begin(); it != m_pr->m_mapRef_Member.end(); it++)
    {
        ...
        if (it->second.m_sig.m_type == mdtFieldDef)
        {
            ...
            CLR_RECORD_FIELDREF *dst = m_tableFieldRef.Alloc(1);
            if (dst == NULL)
                REPORT_NO_MEMORY();

            if (!AllocString(mr.m_name, dst->name, false))
                REPORT_NO_MEMORY();
            if (!AllocSignatureForField(&mr.m_sig.m_sigField, dst->sig))
                REPORT_NO_MEMORY();

            DumpSig(tk, dst->sig, m_tmpSig, (int)(m_tmpSigPtr - m_tmpSig));

            tk2 = m_lookupIDs[mr.m_tr];
            if (CLR_TypeFromTk(tk2) != TBL_TypeRef)
                REPORT_NO_MEMORY();
        ...
        if (it->second.m_sig.m_type == mdtMethodDef)
        {
            ...
            CLR_RECORD_METHODREF *dst = m_tableMethodRef.Alloc(1);
            if (dst == NULL)
                REPORT_NO_MEMORY();

            if (!AllocString(mr.m_name, dst->name, false))
                REPORT_NO_MEMORY();
            if (!AllocSignatureForMethod(&mr.m_sig.m_sigMethod, dst->sig))
                REPORT_NO_MEMORY();
...
HRESULT
WatchAssemblyBuilder::Linker::ProcessTypeDef(MetaData::mdTypeDefList &order)
{
    NANOCLR_HEADER();
    ...
    {
        NANOCLR_CHECK_HRESULT(ResolveTypeDef(it->second, order, resolved));
    }
    ...
    for (itOrder = order.begin(); itOrder != order.end(); itOrder++)
    {
        ...
        CLR_RECORD_TYPEDEF *dst = m_tableTypeDef.Alloc(1);
        if (dst == NULL)
            REPORT_NO_MEMORY();
    }
    ...
    for (itOrder = order.begin(); itOrder != order.end(); itOrder++)
    {
        NANOCLR_CHECK_HRESULT(ProcessTypeDef((mdTypeDef)*itOrder));
    }
...
HRESULT WatchAssemblyBuilder::Linker::ProcessTypeDef(mdTypeDef tdIdx)
{
    ...
    if (!AllocString(td.m_name, dst->name, dst->nameSpace))
        REPORT_NO_MEMORY();
    ...
    {
        case tdNotPublic:
            dst->flags = CLR_RECORD_TYPEDEF::TD_Scope_NotPublic;
            break;
        case tdPublic:
            dst->flags = CLR_RECORD_TYPEDEF::TD_Scope_Public;
            break;
        case tdNestedPublic:
            dst->flags = CLR_RECORD_TYPEDEF::TD_Scope_NestedPublic;
            break;
        case tdNestedPrivate:
            dst->flags = CLR_RECORD_TYPEDEF::TD_Scope_NestedPrivate;
            break;
        case tdNestedFamily:
            dst->flags = CLR_RECORD_TYPEDEF::TD_Scope_NestedFamily;
            break;
        case tdNestedAssembly:
            dst->flags = CLR_RECORD_TYPEDEF::TD_Scope_NestedAssembly;
            break;
        case tdNestedFamANDAssem:
            dst->flags = CLR_RECORD_TYPEDEF::TD_Scope_NestedFamANDAssem;
            break;
        case tdNestedFamORAssem:
            dst->flags = CLR_RECORD_TYPEDEF::TD_Scope_NestedFamORAssem;
            break;
        default:
            wprintf(L"Linker error: wrong visibility mask\n");
            NANOCLR_SET_AND_LEAVE(CLR_E_FAIL);
    }
    ...
    if (td.m_flags & tdSerializable)
        dst->flags |= CLR_RECORD_TYPEDEF::TD_Serializable;
    if (td.m_flags & tdAbstract)
        dst->flags |= CLR_RECORD_TYPEDEF::TD_Abstract;
    if (td.m_flags & tdSealed)
        dst->flags |= CLR_RECORD_TYPEDEF::TD_Sealed;
    if (td.m_flags & tdSpecialName)
        dst->flags |= CLR_RECORD_TYPEDEF::TD_SpecialName;
    if (td.m_flags & tdBeforeFieldInit)
        dst->flags |= CLR_RECORD_TYPEDEF::TD_BeforeFieldInit;
    if (td.m_flags & tdHasSecurity)
        dst->flags |= CLR_RECORD_TYPEDEF::TD_HasSecurity;
    ...
    //
    {
        ...
        if (td.m_name == L"System.DateTime" || td.m_name == L"System.TimeSpan")
        {
            __debugbreak();
        }
        ...
        {
            if (!wcscmp(c_Builtins[n].szName, szName))
                break;
        }
        ...
        else
        {
            switch (dst->flags & CLR_RECORD_TYPEDEF::TD_Semantics_Mask)
            {
                ...
                case CLR_RECORD_TYPEDEF::TD_Semantics_Enum:
                {
                    ...
                    for (MetaData::mdFieldDefListIter itFieldDef = td.m_fields.begin(); itFieldDef != td.m_fields.end();
                         itFieldDef++)
                    {
    ...
    for (itField = td.m_fields.begin(); itField != td.m_fields.end(); itField++)
    {
        ...
        NANOCLR_CHECK_HRESULT(ProcessFieldDef(td, dst, fd, fdStatic));
    }
    NANOCLR_CHECK_HRESULT(m_tableFieldDef.VerifyRange(dst->sFields_First, 0x00FF, L"number", L"static fields"));
    ...
    for (itField = td.m_fields.begin(); itField != td.m_fields.end(); itField++)
    {
        ...
        NANOCLR_CHECK_HRESULT(ProcessFieldDef(td, dst, fd, 0));
    }
    NANOCLR_CHECK_HRESULT(m_tableFieldDef.VerifyRange(dst->iFields_First, 0xFF, L"number", L"instance fields"));
    dst->iFields_Num = (CLR_UINT8)((CLR_IDX)m_tableFieldDef.GetPos() - dst->iFields_First);

    NANOCLR_CHECK_HRESULT(m_tableFieldDef.VerifyRange(0, 0xFFFF, L"number", L"fields"));

    //--//

    lastMethod = m_tableMethodDef.GetPos();
    ...
    for (itMethod = td.m_methods.begin(); itMethod != td.m_methods.end(); itMethod++)
    {
        ...
        NANOCLR_CHECK_HRESULT(ProcessMethodDef(td, dst, md, mdVirtual));
    }
    NANOCLR_CHECK_HRESULT(m_tableMethodDef.VerifyRange(lastMethod, 0xFF, L"number", L"virtual methods"));
    dst->vMethods_Num = (CLR_UINT8)((CLR_IDX)m_tableMethodDef.GetPos() - lastMethod);
    lastMethod += dst->vMethods_Num;
    ...
    for (itMethod = td.m_methods.begin(); itMethod != td.m_methods.end(); itMethod++)
    {
        ...
        NANOCLR_CHECK_HRESULT(ProcessMethodDef(td, dst, md, 0));
    }
    NANOCLR_CHECK_HRESULT(m_tableMethodDef.VerifyRange(lastMethod, 0xFF, L"number", L"instance methods"));
    dst->iMethods_Num = (CLR_UINT8)((CLR_IDX)m_tableMethodDef.GetPos() - lastMethod);
    lastMethod += dst->iMethods_Num;
    ...
    for (itMethod = td.m_methods.begin(); itMethod != td.m_methods.end(); itMethod++)
    {
        ...
        NANOCLR_CHECK_HRESULT(ProcessMethodDef(td, dst, md, mdStatic));
    }
    NANOCLR_CHECK_HRESULT(m_tableMethodDef.VerifyRange(lastMethod, 0xFF, L"number", L"static methods"));
    dst->sMethods_Num = (CLR_UINT8)((CLR_IDX)m_tableMethodDef.GetPos() - lastMethod);
    lastMethod += dst->iMethods_Num;

    //--//

    NANOCLR_CHECK_HRESULT(CheckRange(td.m_interfaces, (CLR_UINT8)0, L"number", L"interfaces"));

    if (!AllocSignatureForInterfaces(td.m_interfaces, dst->interfaces))
        REPORT_NO_MEMORY();
...
HRESULT WatchAssemblyBuilder::Linker::ProcessTypeDef_ByteCode(mdTypeDef tdIdx)
{
    ...
    for (itMethod = td.m_methods.begin(); itMethod != td.m_methods.end(); itMethod++)
    {
        ...
        NANOCLR_CHECK_HRESULT(ProcessMethodDef_ByteCode(td, dst, md, mdVirtual));
    }
    ...
    for (itMethod = td.m_methods.begin(); itMethod != td.m_methods.end(); itMethod++)
    {
        ...
        NANOCLR_CHECK_HRESULT(ProcessMethodDef_ByteCode(td, dst, md, 0));
    }
    ...
    for (itMethod = td.m_methods.begin(); itMethod != td.m_methods.end(); itMethod++)
    {
        ...
        NANOCLR_CHECK_HRESULT(ProcessMethodDef_ByteCode(td, dst, md, mdStatic));
    }
...
HRESULT WatchAssemblyBuilder::Linker::ProcessFieldDef(
    MetaData::TypeDef &td,
    CLR_RECORD_TYPEDEF *tdDst,
    MetaData::FieldDef &fd,
    CLR_UINT32 mode)
{
    NANOCLR_HEADER();

    CLR_RECORD_FIELDDEF *dst;

    if (fd.m_flags & fdLiteral)
    {
        NANOCLR_SET_AND_LEAVE(S_FALSE); // Don't generate constant fields.
    }
    ...
    dst = m_tableFieldDef.Alloc(1);
    if (dst == NULL)
        REPORT_NO_MEMORY();

    if (!AllocString(fd.m_name, dst->name, false))
        REPORT_NO_MEMORY();
    if (!AllocSignatureForField(&fd.m_sig.m_sigField, dst->sig))
        REPORT_NO_MEMORY();
    ...
    {
        case fdPrivateScope:
            dst->flags = CLR_RECORD_FIELDDEF::FD_Scope_PrivateScope;
            break;
        case fdPrivate:
            dst->flags = CLR_RECORD_FIELDDEF::FD_Scope_Private;
            break;
        case fdFamANDAssem:
            dst->flags = CLR_RECORD_FIELDDEF::FD_Scope_FamANDAssem;
            break;
        case fdAssembly:
            dst->flags = CLR_RECORD_FIELDDEF::FD_Scope_Assembly;
            break;
        case fdFamily:
            dst->flags = CLR_RECORD_FIELDDEF::FD_Scope_Family;
            break;
        case fdFamORAssem:
            dst->flags = CLR_RECORD_FIELDDEF::FD_Scope_FamORAssem;
            break;
        case fdPublic:
            dst->flags = CLR_RECORD_FIELDDEF::FD_Scope_Public;
            break;
        default:
            wprintf(L"Linker error: wrong field access mask in ProcessFieldDef\n");
            NANOCLR_SET_AND_LEAVE(CLR_E_FAIL);
    }

    if (fd.m_flags & fdStatic)
        dst->flags |= CLR_RECORD_FIELDDEF::FD_Static;
    if (fd.m_flags & fdInitOnly)
        dst->flags |= CLR_RECORD_FIELDDEF::FD_InitOnly;
    if (fd.m_flags & fdLiteral)
        dst->flags |= CLR_RECORD_FIELDDEF::FD_Literal;
    if (fd.m_flags & fdNotSerialized)
        dst->flags |= CLR_RECORD_FIELDDEF::FD_NotSerialized;

    if (fd.m_flags & fdSpecialName)
        dst->flags |= CLR_RECORD_FIELDDEF::FD_SpecialName;
    if (fd.m_flags & fdHasDefault)
        dst->flags |= CLR_RECORD_FIELDDEF::FD_HasDefault;
    if (fd.m_flags & fdHasFieldRVA)
        dst->flags |= CLR_RECORD_FIELDDEF::FD_HasFieldRVA;
    ...
    if (fd.m_value.size() > 0)
    {
        ...
        if (!FlushSignature(dst->defaultValue, &valueWithSize[0], (int)valueWithSize.size()))
            REPORT_NO_MEMORY();
    }
    else
    {
...
HRESULT WatchAssemblyBuilder::Linker::ProcessMethodDef(
    MetaData::TypeDef &td,
    CLR_RECORD_TYPEDEF *tdDst,
    MetaData::MethodDef &md,
    CLR_UINT32 mode)
{
    NANOCLR_HEADER();

    CLR_RECORD_METHODDEF *dst;
    ...
    {
        case mdPrivateScope:
            dst->flags = CLR_RECORD_METHODDEF::MD_Scope_PrivateScope;
            break;
        case mdPrivate:
            dst->flags = CLR_RECORD_METHODDEF::MD_Scope_Private;
            break;
        case mdFamANDAssem:
            dst->flags = CLR_RECORD_METHODDEF::MD_Scope_FamANDAssem;
            break;
        case mdAssem:
            dst->flags = CLR_RECORD_METHODDEF::MD_Scope_Assem;
            break;
        case mdFamily:
            dst->flags = CLR_RECORD_METHODDEF::MD_Scope_Family;
            break;
        case mdFamORAssem:
            dst->flags = CLR_RECORD_METHODDEF::MD_Scope_FamORAssem;
            break;
        case mdPublic:
            dst->flags = CLR_RECORD_METHODDEF::MD_Scope_Public;
            break;
        default:
            wprintf(L"Linker error: wrong member access mask \n");
            NANOCLR_SET_AND_LEAVE(CLR_E_FAIL);
    }

    if (md.m_flags & mdStatic)
        dst->flags |= CLR_RECORD_METHODDEF::MD_Static;
    if (md.m_flags & mdFinal)
        dst->flags |= CLR_RECORD_METHODDEF::MD_Final;
    if (md.m_flags & mdVirtual)
        dst->flags |= CLR_RECORD_METHODDEF::MD_Virtual;
    if (md.m_flags & mdHideBySig)
        dst->flags |= CLR_RECORD_METHODDEF::MD_HideBySig;

    if (md.m_flags & mdNewSlot)
        dst->flags |= CLR_RECORD_METHODDEF::MD_NewSlot;

    if (md.m_flags & mdAbstract)
        dst->flags |= CLR_RECORD_METHODDEF::MD_Abstract;
    if (md.m_flags & mdSpecialName)
        dst->flags |= CLR_RECORD_METHODDEF::MD_SpecialName;

    if (md.m_flags & mdHasSecurity)
        dst->flags |= CLR_RECORD_METHODDEF::MD_HasSecurity;
    if (md.m_flags & mdRequireSecObject)
        dst->flags |= CLR_RECORD_METHODDEF::MD_RequireSecObject;
    ...
    if (md.m_implFlags & miRuntime)
    {
        ...
        {
            wprintf(L"Linker error when processing method definition: It should be "
                    L"MulticastDelegate or Delegate\n");
            NANOCLR_SET_AND_LEAVE(CLR_E_FAIL);
        }

        if (!wcscmp(szName, L".ctor"))
            dst->flags |= CLR_RECORD_METHODDEF::MD_DelegateConstructor;
        if (!wcscmp(szName, L"Invoke"))
            dst->flags |= CLR_RECORD_METHODDEF::MD_DelegateInvoke;
        if (!wcscmp(szName, L"BeginInvoke"))
            dst->flags |= CLR_RECORD_METHODDEF::MD_DelegateBeginInvoke;
        if (!wcscmp(szName, L"EndInvoke"))
            dst->flags |= CLR_RECORD_METHODDEF::MD_DelegateEndInvoke;
    }

    if (IsMdInstanceInitializerW(md.m_flags, szName))
        dst->flags |= CLR_RECORD_METHODDEF::MD_Constructor;
    if (IsMdClassConstructorW(md.m_flags, szName))
        dst->flags |= CLR_RECORD_METHODDEF::MD_StaticConstructor;

    if (!wcscmp(szName, L"Finalize") && md.m_method.m_retValue.m_opt == ELEMENT_TYPE_VOID &&
        md.m_method.m_lstParams.size() == 0)
    {
    ...
    if (!AllocString(md.m_name, dst->name, false))
        REPORT_NO_MEMORY();
    if (!AllocSignatureForMethod(&md.m_method, dst->sig))
        REPORT_NO_MEMORY();
    ...
    dst->numArgs = (CLR_UINT8)md.m_method.m_lstParams.size();
    if ((md.m_flags & mdStatic) == 0)
        dst->numArgs++; // Include the 'this' pointer in the number of arguments.
    ...
    {
        NANOCLR_CHECK_HRESULT(CheckRange(md.m_vars.m_lstVars, dst->numLocals, L"number", L"locals"));

        dst->numLocals = (CLR_UINT8)md.m_vars.m_lstVars.size();

        if (!AllocSignatureForLocalVar(md.m_vars, dst->locals))
            REPORT_NO_MEMORY();
    }
...
HRESULT WatchAssemblyBuilder::Linker::ProcessMethodDef_ByteCode(
    MetaData::TypeDef &td,
    CLR_RECORD_TYPEDEF *tdDst,
    MetaData::MethodDef &md,
    CLR_UINT32 mode)
{
    ...
    dst = m_tableMethodDef.GetRecordAt(CLR_DataFromTk(m_lookupIDs[md.m_md]));
    ...
    else if (md.m_byteCode.m_opcodes.size() > 0)
    {
        ...
        NANOCLR_CHECK_HRESULT(md.m_byteCode.ConvertTokens(m_lookupIDs));
        NANOCLR_CHECK_HRESULT(md.m_byteCode.GenerateOldIL(code));
        ...
        if (stackDepth > md.m_maxStack)
        {
            ...
            // It's ok if we need less (apparently, sometimes the c# compiler appears
            // to ask for more than necessary)
            wprintf(L"Bad stack depth");
            NANOCLR_SET_AND_LEAVE(CLR_E_FAIL);
        }

        NANOCLR_CHECK_HRESULT(CheckRange(stackDepth, dst->lengthEvalStack, L"depth", L"stack"));
        ...
        BYTE *byteCodeDst = m_tableByteCode.Alloc(byteCodeLen);
        if (byteCodeDst == NULL)
            REPORT_NO_MEMORY();
        ...
        if (numExceptions > 0)
        {
...

            {
                ...
                NANOCLR_CHECK_HRESULT(lookupEh.GenerateOutput(tableEh));

                NANOCLR_CHECK_HRESULT(tableEh.CopyTo(m_tableByteCode, start));
            }

            byteCodeDst = m_tableByteCode.Alloc(1);
            if (byteCodeDst == NULL)
                REPORT_NO_MEMORY();
...
WatchAssemblyBuilder::Linker::CustomAttributeId::CustomAttributeId(Linker *lk, MetaData::CustomAttribute &ca) : m_ca(ca)
{
    ...
    {
        case mdtTypeDef:
            m_id.ownerType = TBL_TypeDef;
            break;
        case mdtFieldDef:
            m_id.ownerType = TBL_FieldDef;
            break;
        case mdtMethodDef:
            m_id.ownerType = TBL_MethodDef;
            break;
    }
...
HRESULT WatchAssemblyBuilder::Linker::ProcessAttribute()
{
    ...
    for (CustomAttributeIdSetIter itS = set.begin(); itS != set.end(); itS++)
    {
        ...
        if (!AllocSignatureForAttribute(id.m_ca, id.m_id.data))
            REPORT_NO_MEMORY();

        CLR_RECORD_ATTRIBUTE *dst = m_tableAttribute.Alloc(1);
        if (dst == NULL)
            REPORT_NO_MEMORY();
...
static HRESULT AlignToWORD(WatchAssemblyBuilder::CQuickRecord<BYTE> &buf)
{
    ...
    if (diff)
    {
        if (buf.Alloc(sizeof(CLR_UINT32) - diff) == NULL)
            REPORT_NO_MEMORY();
    }
...
HRESULT WatchAssemblyBuilder::Linker::ProcessResource()
{
    ...
    for (CLR_RT_StringSet::iterator it = m_pr->m_resources.begin(); it != m_pr->m_resources.end(); it++)
    {
        ...
        NANOCLR_CHECK_HRESULT(CLR_RT_FileStore::LoadFile(str.c_str(), buf));
        ...
        if (ptr > ptrMax)
            NANOCLR_MSG_SET_AND_LEAVE(CLR_E_FAIL, L"Linker error when processing resource: pointer overflowed\n");
        ...
        resourceFile = m_tableResourceFile.Alloc(1);
        FAULT_ON_NULL(resourceFile);
        ...
        if (!AllocString(strName, resourceFile->name, true))
            NANOCLR_SET_AND_LEAVE(CLR_E_OUT_OF_MEMORY);
        ...
        for (CLR_UINT32 iResource = 0; iResource < fileHeader->numberOfResources; iResource++)
        {
            ...
            if (ptr > ptrMax)
            {
                wprintf(L"Linker error when processing resource: resource header "
                        L"overflow \n");
                NANOCLR_SET_AND_LEAVE(CLR_E_FAIL);
            }
            ptrResourceData = ptr;
            ...
            if (iResource > 0)
            {
                ...
                {
                    wprintf(L"Linker error when processing resource: wrong resource "
                            L"header order\n");
                    NANOCLR_SET_AND_LEAVE(CLR_E_FAIL);
                }
            }
            ...
            resourceDst = m_tableResource.Alloc(1);
            FAULT_ON_NULL(resourceDst);
            ...
            // write resource data
            dataDst = m_tableResourceData.Alloc(resource->size);
            FAULT_ON_NULL(dataDst);
    ...
    {
        // Dump sentinel at end.  This is necessary to accurately compute the size
        // of the last resource The MMP will pad tables to get to 32-bit boundaries
        resourceDst = m_tableResource.Alloc(1);
        FAULT_ON_NULL(resourceDst);
...
HRESULT WatchAssemblyBuilder::Linker::ProcessTypeSpec()
{
    ...
    for (MetaData::TypeSpecMapIter it = m_pr->m_mapSpec_Type.begin(); it != m_pr->m_mapSpec_Type.end(); it++)
    {
        ...
        CLR_RECORD_TYPESPEC *dst = m_tableTypeSpec.Alloc(1);
        if (dst == NULL)
            REPORT_NO_MEMORY();

        if (!AllocSignatureForTypeSpec(&ts.m_sig, dst->sig))
            REPORT_NO_MEMORY();
    }
...
HRESULT
WatchAssemblyBuilder::Linker::ResolveTypeRef(
    MetaData::TypeRef &tr,
    MetaData::mdTypeRefList &order,
    MetaData::mdTokenSet &resolved)
{
    ...
    {
        NANOCLR_CHECK_HRESULT(m_pr->CheckTokenPresence(tr.m_scope));

        if (TypeFromToken(tr.m_scope) == mdtTypeRef)
        {
            if (MetaData::IsTokenPresent(resolved, tr.m_scope) == false)
            {
                NANOCLR_CHECK_HRESULT(ResolveTypeRef(m_pr->m_mapRef_Type[tr.m_scope], order, resolved));
            }
        }
    }
...
HRESULT
WatchAssemblyBuilder::Linker::ResolveTypeDef(
    MetaData::TypeDef &td,
    MetaData::mdTypeDefList &order,
    MetaData::mdTokenSet &resolved)
{
    ...
    {
        NANOCLR_CHECK_HRESULT(m_pr->CheckTokenPresence(td.m_enclosingClass));
        ...
        if (TypeFromToken(td.m_enclosingClass) == mdtTypeDef)
        {
            if (MetaData::IsTokenPresent(resolved, td.m_enclosingClass) == false)
            {
                NANOCLR_CHECK_HRESULT(
                    ResolveTypeDef(m_pr->m_mapDef_Type.find(td.m_enclosingClass)->second, order, resolved));
            }
        }
    }
    ...
    if (IsNilToken(td.m_extends)) // Nested class.
    {
        ...
        {
            //
            // Ops, this should not happen, unless it's the special case for root of
            // hierarchy...
            //
            if (td.m_name != L"System.Object")
            {
                wprintf(L"Linker error when resolving type def: type is not "
                        L"System.Object \n");
                NANOCLR_SET_AND_LEAVE(CLR_E_FAIL);
            }
        }
    ...
    {
        NANOCLR_CHECK_HRESULT(m_pr->CheckTokenPresence(td.m_extends));

        if (TypeFromToken(td.m_extends) == mdtTypeDef)
        {
            if (MetaData::IsTokenPresent(resolved, td.m_extends) == false)
            {
                NANOCLR_CHECK_HRESULT(ResolveTypeDef(m_pr->m_mapDef_Type.find(td.m_extends)->second, order, resolved));
            }
        }
    }
    ...
    for (MetaData::mdInterfaceImplListIter it = td.m_interfaces.begin(); it != td.m_interfaces.end(); it++)
    {
        ...
        if (TypeFromToken(ii.m_itf) == mdtTypeDef)
        {
            ...
            {
                NANOCLR_CHECK_HRESULT(ResolveTypeDef(m_pr->m_mapDef_Type.find(ii.m_itf)->second, order, resolved));
            }
        }
    }
...
template <typename T>
HRESULT AppendToAssembly(
    WatchAssemblyBuilder::CQuickRecord<T> &input,
    WatchAssemblyBuilder::CQuickRecord<BYTE> &output,
    ...
    LPCWSTR streamName)
{
    ...
    NANOCLR_CHECK_HRESULT(input.CopyTo(output, header.startOfTables[tbl], maxSize));

    sizeOld = output.Size();

    NANOCLR_CHECK_HRESULT(AlignToWORD(output));
...
HRESULT WatchAssemblyBuilder::Linker::EmitData(CQuickRecord<BYTE> &buf, CLR_RECORD_ASSEMBLY &headerSrc)
{
    ...
    header = (CLR_RECORD_ASSEMBLY *)buf.Alloc(sizeof(*header));
    if (header == NULL)
        REPORT_NO_MEMORY();

    //--//

    NANOCLR_CHECK_HRESULT(AppendToAssembly(
        m_tableAssemblyRef,
        buf,
        headerSrc,
        TBL_AssemblyRef,
        CLR_MaxStreamSize_AssemblyRef,
        L"AssemblyRef"));
    NANOCLR_CHECK_HRESULT(
        AppendToAssembly(m_tableTypeRef, buf, headerSrc, TBL_TypeRef, CLR_MaxStreamSize_TypeRef, L"TypeRef"));
    NANOCLR_CHECK_HRESULT(
        AppendToAssembly(m_tableFieldRef, buf, headerSrc, TBL_FieldRef, CLR_MaxStreamSize_FieldRef, L"FieldRef"));
    NANOCLR_CHECK_HRESULT(
        AppendToAssembly(m_tableMethodRef, buf, headerSrc, TBL_MethodRef, CLR_MaxStreamSize_MethodRef, L"MethodRef"));
    NANOCLR_CHECK_HRESULT(
        AppendToAssembly(m_tableTypeDef, buf, headerSrc, TBL_TypeDef, CLR_MaxStreamSize_TypeDef, L"TypeDef"));
    NANOCLR_CHECK_HRESULT(
        AppendToAssembly(m_tableFieldDef, buf, headerSrc, TBL_FieldDef, CLR_MaxStreamSize_FieldDef, L"FieldDef"));
    NANOCLR_CHECK_HRESULT(
        AppendToAssembly(m_tableMethodDef, buf, headerSrc, TBL_MethodDef, CLR_MaxStreamSize_MethodDef, L"MethodDef"));
    NANOCLR_CHECK_HRESULT(AppendToAssembly(
        m_tableAttribute,
        buf,
        headerSrc,
        TBL_Attributes,
        CLR_MaxStreamSize_Attributes,
        L"Attributes"));
    NANOCLR_CHECK_HRESULT(
        AppendToAssembly(m_tableTypeSpec, buf, headerSrc, TBL_TypeSpec, CLR_MaxStreamSize_TypeSpec, L"TypeSpec"));
    NANOCLR_CHECK_HRESULT(
        AppendToAssembly(m_tableResource, buf, headerSrc, TBL_Resources, CLR_MaxStreamSize_Resources, L"Resources"));
    NANOCLR_CHECK_HRESULT(AppendToAssembly(
        m_tableResourceData,
        buf,
        headerSrc,
        TBL_ResourcesData,
        CLR_MaxStreamSize_ResourcesData,
        L"ResourcesData"));
    NANOCLR_CHECK_HRESULT(
        AppendToAssembly(m_tableString, buf, headerSrc, TBL_Strings, CLR_MaxStreamSize_Strings, L"Strings"));
    NANOCLR_CHECK_HRESULT(AppendToAssembly(
        m_tableSignature,
        buf,
        headerSrc,
        TBL_Signatures,
        CLR_MaxStreamSize_Signatures,
        L"Signatures"));
    NANOCLR_CHECK_HRESULT(
        AppendToAssembly(m_tableByteCode, buf, headerSrc, TBL_ByteCode, CLR_MaxStreamSize_ByteCode, L"ByteCode"));
    NANOCLR_CHECK_HRESULT(AppendToAssembly(
        m_tableResourceFile,
        buf,
        headerSrc,
        TBL_ResourcesFiles,
        CLR_MaxStreamSize_ResourcesFiles,
        L"ResourcesFiles"));
...
HRESULT WatchAssemblyBuilder::Linker::Generate(
    CQuickRecord<BYTE> &buf,
    bool patch_fReboot,
    std::wstring *patch_szNative)
{
    NANOCLR_HEADER();

    CLR_RECORD_ASSEMBLY header;
    NANOCLR_CLEAR(header);
    ...
    if (m_pr)
    {
        ...
        if (!AllocString(m_pr->m_assemblyName, header.assemblyName, false))
            REPORT_NO_MEMORY();
    }
    ...
    if (patch_szNative)
    {
        ...
        NANOCLR_CHECK_HRESULT(CLR_RT_FileStore::LoadFile(patch_szNative->c_str(), buffer));

        header.patchEntryOffset = (CLR_UINT32)m_tableResourceData.GetPos();

        BYTE *res = m_tableResourceData.Alloc(buffer.size());
        if (res == NULL)
            REPORT_NO_MEMORY();

        memcpy(res, &buffer[0], buffer.size());

        NANOCLR_CHECK_HRESULT(AlignToWORD(m_tableResourceData));
    }
    else
    {
    ...
    NANOCLR_CHECK_HRESULT(EmitData(buf, header));
    ...
    //    if(CRYPTO_SUCCESS == ::Crypto_SignBuffer( (BYTE*)buf.Ptr(),
    //    (int)buf.Size(), &key, result, sizeof(result) ))
    //    {
    //        RSAKey key2;

    //        if(CRYPTO_SUCCESS == ::Crypto_PublicKeyFromModulus( &key.module,
    //        &key2 ))
    //        {
    //            BYTE unmanagedHash     [HASH_SIZE];
    //            BYTE decryptedSignature[HASH_SIZE];

    //            if(::Crypto_GetHash( (BYTE*)buf.Ptr(), (int)buf.Size(),
    //            unmanagedHash, HASH_SIZE ))
    //            {
    //                if(CRYPTO_SUCCESS == ::Crypto_RSADecrypt( &key2,
    //                decryptedSignature, HASH_SIZE, result, sizeof(result) ))
    //                {
    //                    if(memcmp( decryptedSignature, unmanagedHash, HASH_SIZE
    //                    ) == 0)
    //                    {
    //                        int i = 1;
    //                    }
...
HRESULT WatchAssemblyBuilder::Linker::DumpPdbx(std::wstring szFileNamePE)
{
    NANOCLR_HEADER();
    ...
    if (!m_pr)
        NANOCLR_MSG_SET_AND_LEAVE(CLR_E_FAIL, L"Linker error when dumping pdbx: MDP can't be null\n");

    NANOCLR_CHECK_HRESULT(GetFullPath(m_pr->m_assemblyFile, NULL, &strAssemblyFile));

    NANOCLR_CHECK_HRESULT(xml.New(L"PdbxFile"));
    ...
    for (MetaData::TypeDefMapIter itTypeDef = m_pr->m_mapDef_Type.begin(); itTypeDef != m_pr->m_mapDef_Type.end();
         itTypeDef++)
    {
        ...
        for (MetaData::mdMethodDefListIter itMethod = td.m_methods.begin(); itMethod != td.m_methods.end(); itMethod++)
        {
            ...
            if (md.m_byteCode.m_opcodes.size() != md.m_byteCodeOriginal.m_opcodes.size())
                NANOCLR_MSG_SET_AND_LEAVE(CLR_E_FAIL, L"Linker error when dumping pdbx: op codes size is different\n");
            ...
            for (size_t i = 0; i < md.m_byteCode.m_opcodes.size(); i++)
            {
                ...
                if (op.m_op != opOriginal.m_op || ipDiffNew < ipDiff)
                    NANOCLR_MSG_SET_AND_LEAVE(CLR_E_FAIL, L"Linker error when dumping pdbx: op codes are different\n");
    ...
    NANOCLR_CHECK_HRESULT(xml.Save(strFilePdbx.c_str()));
...
static HRESULT local_DumpVersion(CLR_XmlUtil &xml, IXMLDOMNode *pNode, LPCWSTR szName, CLR_RECORD_VERSION &ver)
{
    ...
    NANOCLR_CHECK_HRESULT(xml.CreateNode(szName, &pNodeVersion, pNode));

    NANOCLR_CHECK_HRESULT(xml.PutValue(L"Major", _variant_t(ver.iMajorVersion), fFound, pNodeVersion));
    NANOCLR_CHECK_HRESULT(xml.PutValue(L"Minor", _variant_t(ver.iMinorVersion), fFound, pNodeVersion));
    NANOCLR_CHECK_HRESULT(xml.PutValue(L"Build", _variant_t(ver.iBuildNumber), fFound, pNodeVersion));
    NANOCLR_CHECK_HRESULT(xml.PutValue(L"Revision", _variant_t(ver.iRevisionNumber), fFound, pNodeVersion));
