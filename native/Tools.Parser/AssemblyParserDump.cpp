...
void MetaData::Parser::TokenToString(mdToken tk, std::wstring &str)
{
    ...
    switch (TypeFromToken(tk))
    {
        case mdtTypeRef:
        {
            TypeRefMapIter it = m_mapRef_Type.find(tk);
            if (it == m_mapRef_Type.end())
                break;
        ...
        case mdtTypeDef:
        {
            TypeDefMapIter it = m_mapDef_Type.find(tk);
            if (it == m_mapDef_Type.end())
                break;
        ...
        case mdtFieldDef:
        {
            FieldDefMapIter it = m_mapDef_Field.find(tk);
        ...
        case mdtMethodDef:
        {
            MethodDefMapIter it = m_mapDef_Method.find(tk);
        ...
        {
            InterfaceImplMapIter it = m_mapDef_Interface.find(tk);
            if (it == m_mapDef_Interface.end())
                break;
            InterfaceImpl &ii = it->second;

            std::wstring cls;
            TokenToString(ii.m_td, cls);
            std::wstring itf;
            TokenToString(ii.m_itf, itf);
        ...
        case mdtMemberRef:
        {
            MemberRefMapIter it = m_mapRef_Member.find(tk);
            if (it == m_mapRef_Member.end())
                break;
        ...
        case mdtModuleRef:
        {
            ModuleRefMapIter it = m_mapRef_Module.find(tk);
            if (it == m_mapRef_Module.end())
                break;
        ...
        case mdtTypeSpec:
        {
            TypeSpecMapIter it = m_mapSpec_Type.find(tk);
            if (it == m_mapSpec_Type.end())
                break;
        ...
        case mdtAssemblyRef:
        {
            AssemblyRefMapIter it = m_mapRef_Assembly.find(tk);
            if (it == m_mapRef_Assembly.end())
                break;
        ...
        case mdtString:
        {
            UserStringMapIter it = m_mapDef_String.find(tk);
            if (it == m_mapDef_String.end())
                break;
...
void MetaData::Parser::Dump_PrintSigForType(TypeSignature &sig)
{
    switch (sig.m_opt)
    {
        case ELEMENT_TYPE_VOID:
            fwprintf(m_output, L"VOID");
            break;
        case ELEMENT_TYPE_BOOLEAN:
            fwprintf(m_output, L"BOOLEAN");
            break;
        case ELEMENT_TYPE_CHAR:
            fwprintf(m_output, L"CHAR");
            break;
        case ELEMENT_TYPE_I1:
            fwprintf(m_output, L"I1");
            break;
        case ELEMENT_TYPE_U1:
            fwprintf(m_output, L"U1");
            break;
        case ELEMENT_TYPE_I2:
            fwprintf(m_output, L"I2");
            break;
        case ELEMENT_TYPE_U2:
            fwprintf(m_output, L"U2");
            break;
        case ELEMENT_TYPE_I4:
            fwprintf(m_output, L"I4");
            break;
        case ELEMENT_TYPE_U4:
            fwprintf(m_output, L"U4");
            break;
        case ELEMENT_TYPE_I8:
            fwprintf(m_output, L"I8");
            break;
        case ELEMENT_TYPE_U8:
            fwprintf(m_output, L"U8");
            break;
        case ELEMENT_TYPE_R4:
            fwprintf(m_output, L"R4");
            break;
        case ELEMENT_TYPE_R8:
            fwprintf(m_output, L"R8");
            break;
        case ELEMENT_TYPE_STRING:
            fwprintf(m_output, L"STRING");
            break;
        case ELEMENT_TYPE_PTR:
            fwprintf(m_output, L"PTR ");
            break;
        case ELEMENT_TYPE_BYREF:
            fwprintf(m_output, L"BYREF ");
            break;
        case ELEMENT_TYPE_VALUETYPE:
            fwprintf(m_output, L"VALUETYPE [%08x]", sig.m_token);
            break;
        case ELEMENT_TYPE_CLASS:
            fwprintf(m_output, L"CLASS [%08x]", sig.m_token);
            break;
        case ELEMENT_TYPE_ARRAY:
            fwprintf(m_output, L"ARRAY!!");
            break;
        case ELEMENT_TYPE_TYPEDBYREF:
            fwprintf(m_output, L"TYPEDBYREF!!");
            break;
        case ELEMENT_TYPE_I:
            fwprintf(m_output, L"I");
            break;
        case ELEMENT_TYPE_U:
            fwprintf(m_output, L"U");
            break;
        case ELEMENT_TYPE_FNPTR:
            fwprintf(m_output, L"FNPTR!!");
            break;
        case ELEMENT_TYPE_OBJECT:
            fwprintf(m_output, L"OBJECT");
            break;
        case ELEMENT_TYPE_SZARRAY:
            fwprintf(m_output, L"SZARRAY ");
            break;

        case ELEMENT_TYPE_SENTINEL:
            fwprintf(m_output, L"SENTINEL!!");
            break;
        case ELEMENT_TYPE_PINNED:
            fwprintf(m_output, L"PINNED!!");
            break;
    }

    if (sig.m_sub)
        Dump_PrintSigForType(*sig.m_sub);
}
...
void MetaData::Parser::Dump_EnumTypeDefs(bool fNoByteCode)
{
    for (TypeDefMapIter it = m_mapDef_Type.begin(); it != m_mapDef_Type.end(); it++)
    {
        ...
        fwprintf(
            m_output,
            L"TypeDefProps [%08x]: Flags: %08x Extends: %08x Enclosed: %08x '%s'\n",
            td.m_td,
            td.m_flags,
            td.m_extends,
            td.m_enclosingClass,
        ...
        for (mdFieldDefListIter it1 = td.m_fields.begin(); it1 != td.m_fields.end(); it1++)
        {
            ...
            fwprintf(
                m_output,
                L"    FieldDefProps [%08x]: Attr: %08x Flags: %08x '%s' [",
                fd.m_fd,
                fd.m_attr,
                fd.m_flags,
                fd.m_name.c_str());
        ...
        for (mdMethodDefListIter it2 = td.m_methods.begin(); it2 != td.m_methods.end(); it2++)
        {
            ...
            fwprintf(
                m_output,
                L"    MethodDefProps [%08x]: Flags: %08x Impl: %08x RVA: %08x '%s' [",
                md.m_md,
                md.m_flags,
                md.m_implFlags,
                md.m_RVA,
...

            {
                for (size_t i = 0; i < md.m_byteCode.m_exceptions.size(); i++)
                {
                    ...
                    if (leb.m_Flags == COR_ILEXCEPTION_CLAUSE_FILTER)
                    {
                        fwprintf(
                            m_output,
                            L"        EH: %02x %08x->%08x F:%08x %08x->%08x\n",
                            leb.m_Flags,
                            leb.m_TryOffset,
...
void MetaData::Parser::Dump_EnumCustomAttributes()
{
    for (CustomAttributeMapIter itCA = m_mapDef_CustomAttribute.begin(); itCA != m_mapDef_CustomAttribute.end(); itCA++)
    {
        ...
        if (ca.m_valuesVariable.size())
        {
            ...
            for (CustomAttribute::ValueMapIter itV = ca.m_valuesVariable.begin(); itV != ca.m_valuesVariable.end();
                 itV++)
            {
