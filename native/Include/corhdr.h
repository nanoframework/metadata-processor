...
typedef struct IMAGE_COR20_HEADER
{
    ...
    IMAGE_DATA_DIRECTORY CodeManagerTable; // I believe that this is unused (was
                                           // for managed native code)
...
// The predefined name for deleting a typeDef,MethodDef, FieldDef, Property and
// Event
...
typedef enum CorMethodImpl
{
    ...
    miPreserveSig = 0x0080, // Indicates method sig is not to be mangled to do
                            // HRESULT conversion.
...
typedef enum CorPinvokeMap
{
    ...
    pmCallConvWinapi = 0x0100, // Pinvoke will use native callconv appropriate to
...
typedef enum CorElementType
{
    ...
    ELEMENT_TYPE_FNPTR = 0x1B,   // FNPTR <complete sig for the function including
                                 // calling convention>
    ELEMENT_TYPE_OBJECT = 0x1C,  // Shortcut for System.Object
    ELEMENT_TYPE_SZARRAY = 0x1D, // Shortcut for single dimension zero lower bound
                                 // array SZARRAY <type>
...
typedef enum CorSerializationType
{
    ...
    SERIALIZATION_TYPE_SZARRAY = ELEMENT_TYPE_SZARRAY, // Shortcut for single dimension zero lower bound
                                                       // array
...
typedef enum CorCallingConvention
{
    ...
    IMAGE_CEE_CS_CALLCONV_GENERIC = 0x10,      // Generic method sig with explicit number of type arguments
...
typedef enum CorNativeType
{
    ...
    NATIVE_TYPE_CUSTOMMARSHALER = 0x2c, // Custom marshaler native type. This must be followed
                                        ...
                                        // Or
                                        // "{Native type GUID}/0Custom marshaler type name/0Optional
                                        // cookie/0"

    NATIVE_TYPE_ERROR = 0x2d, // This native type coupled with ELEMENT_TYPE_I4
                              // will map to VT_HRESULT
                              // COMINTEROP
...
/***********************************************************************************/
/* If COR_ILMETHOD_SECT_HEADER::Kind() = CorILMethod_Sect_EHTable then the
attribute is a list of exception handling clauses.  There are two formats, fat
or small
*/
typedef enum CorExceptionFlag // defintitions for the Flags field below (for
                              // both big and small)
{
    ...
    COR_ILEXCEPTION_CLAUSE_DUPLICATED = 0x0008, // duplicated clase..  this clause was duplicated down to a
...
typedef struct IMAGE_COR_ILMETHOD_FAT
{
    ...
    unsigned MaxStack : 16;     // maximum number of items (I4, I, I8, obj ...), on
                                // the operand stack
    DWORD CodeSize;             // size of the code
    mdSignature LocalVarSigTok; // token that indicates the signature of the local
                                // vars (0 means none)
...
{
    // default behavior is to always perform TypeRef to TypeDef and MemberRef to
    // MethodDef/FieldDef optimization
...
{
    // default behavior is to notify TypeRef, MethodDef, MemberRef, and FieldDef
    // token remaps
...
{
    // default behavior is to have thread safety turn off. This means that
    // MetaData APIs will not take reader/writer lock. Clients is responsible to
    // make sure the properly thread synchornization when using MetaData APIs.
...
#define mdtGenericPar mdtGenericParam
    ...
    mdtName = 0x71000000,     //
    mdtBaseType = 0x72000000, // Leave this on the high end value. This does not
                              // correspond to metadata table
} CorTokenType;
...
typedef enum CorOpenFlags
{
    ...
    ofCopyMemory = 0x00000002, // Open scope with memory. Ask metadata to maintain
...
typedef enum CorAttributeTargets
{
    ...
    catAll = catAssembly | catModule | catClass | catStruct | catEnum | catConstructor | catMethod | catProperty |
             catField | catEvent | catInterface | catParameter | catDelegate | catGenericParameter,
    catClassMembers = catClass | catStruct | catEnum | catConstructor | catMethod | catProperty | catField | catEvent |
...
#define INTEROP_IMPORTEDFROMTYPELIB_SIG                                                                                \
    {IMAGE_CEE_CS_CALLCONV_DEFAULT_HASTHIS, 1, ELEMENT_TYPE_VOID, ELEMENT_TYPE_STRING}
...
#define INTEROP_BESTFITMAPPING_SIG                                                                                     \
    {IMAGE_CEE_CS_CALLCONV_DEFAULT_HASTHIS, 2, ELEMENT_TYPE_VOID, ELEMENT_TYPE_BOOLEAN, ELEMENT_TYPE_BOOLEAN}
...
#define INTEROP_TYPELIBVERSION_SIG                                                                                     \
    {IMAGE_CEE_CS_CALLCONV_DEFAULT_HASTHIS, 2, ELEMENT_TYPE_VOID, ELEMENT_TYPE_I2, ELEMENT_TYPE_I2}
