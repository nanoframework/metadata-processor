...
#define REPARSER_TABLE_ENUM_BEGIN(obj, src)                                                                            \
    {                                                                                                                  \
        for (MetaData::Reparser::TokenToObjectIter it = obj->m_lookupTokenToObject.begin();                            \
             it != obj->m_lookupTokenToObject.end();                                                                   \
             it++)                                                                                                     \
        {                                                                                                              \
...
struct BaseTokenPtr
{
    ...
    operator BaseToken *() const
    {
        return (BaseToken *)m_ptr;
    }

    BaseToken *operator->() const
    {
        return (BaseToken *)m_ptr;
    }
    ...
    {
        if (m_ptr && m_ptr->GetTableOfInstance() == T::GetTable())
            return (T *)m_ptr;
...
struct TypeSignaturePtr
{
    ...
    operator TypeSignature *() const
    {
        return (TypeSignature *)m_ptr;
    }
...
struct AssemblyRef : public BaseToken
{
    ...
    virtual CLR_TABLESENUM GetTableOfInstance() const
    {
        return GetTable();
    }

    static CLR_TABLESENUM GetTable()
    {
        return TBL_AssemblyRef;
    }
...
class Assembly : public AssemblyRef
{
  ...
  public:
    BaseToken *Parse(CLR_RT_Assembly *assm, mdToken tk)
    {
        return Parse(assm, tk, NULL);
    }
