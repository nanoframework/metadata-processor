...
//
//*****************************************************************************
template <DWORD SIZE, DWORD INCREMENT> class CQuickMemoryBase
{
  public:
    void Init()
    {
...
HRESULT _CountBytesOfOneArg(PCCOR_SIGNATURE pbSig, ULONG *pcbTotal);
...
typedef struct tagCOR_ILMETHOD_SECT_FAT : IMAGE_COR_ILMETHOD_SECT_FAT
{
    ...
    unsigned GetDataSize() const
    {
        /* return DataSize; */
        BYTE *p = (BYTE *)this;
        return ((unsigned)*(p + 1)) | (((unsigned)*(p + 2)) << 8) | (((unsigned)*(p + 3)) << 16);
    }
...
struct COR_ILMETHOD_SECT
{
    ...
    const COR_ILMETHOD_SECT *Next() const
    {
        if (!More())
            return (0);
        if (IsFat())
            return (((COR_ILMETHOD_SECT *)&AsFat()->Data()[AsFat()->GetDataSize()])->Align());
    ...
    {
        // Work around a bug where VB doesn't specify the size correctly for EH
        // sections
...
extern "C"
{
    IMAGE_COR_ILMETHOD_SECT_EH_CLAUSE_FAT
        *__stdcall SectEH_EHClause(void *pSectEH, unsigned idx, IMAGE_COR_ILMETHOD_SECT_EH_CLAUSE_FAT *buff);
...
struct COR_ILMETHOD_SECT_EH : public COR_ILMETHOD_SECT
{
    ...
    const IMAGE_COR_ILMETHOD_SECT_EH_CLAUSE_FAT *EHClause(unsigned idx, IMAGE_COR_ILMETHOD_SECT_EH_CLAUSE_FAT *buff)
        const
    {
...
typedef struct tagCOR_ILMETHOD_FAT : IMAGE_COR_ILMETHOD_FAT
{
    ...
    bool IsFat() const
    {
        /* return((IMAGE_COR_ILMETHOD_FAT::GetFlags() & CorILMethod_FormatMask) ==
    ...
    const COR_ILMETHOD_SECT *GetSect() const
    {
        ...
        if (!(*(BYTE *)this & CorILMethod_MoreSects))
            return (0);
...
extern "C"
{
    ...
    unsigned __stdcall IlmethodEmit(unsigned size, COR_ILMETHOD_FAT *header, BOOL moreSections, BYTE *outBuff);
}
...
struct COR_ILMETHOD
{
    ...
    // emit the header (bestFormat) return amount emitted
    unsigned static Emit(unsigned size, const COR_ILMETHOD_FAT *header, bool moreSections, BYTE *outBuff)
    {
...
class COR_ILMETHOD_DECODER : public COR_ILMETHOD_FAT
{
  public:
    ...
    COR_ILMETHOD_DECODER(COR_ILMETHOD *header, void *pInternalImport, DecoderStatus *wbStatus);
