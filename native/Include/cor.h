...
extern "C"
{
    ...
    //  bind to CLSID_MetaDataDispenser.
    EXTERN_GUID(
...
#define MetaDataSetENC MetaDataSetUpdate
    ...
    // Use this guid in SetOption to indicate if the import enumerator should skip
    // over delete items or not. The default is yes.
    //
    ...
    // Use this guid in the SetOption if compiler wants to have MetaData API to take
    // reader/writer lock
    //
    ...
    // Use this guid in the SetOption if compiler wants error when some tokens are
    // emitted out of order {1547872D-DC03-11d2-9420-0000F8083460}
    EXTERN_GUID(MetaDataErrorIfEmitOutOfOrder, 0x1547872d, 0xdc03, 0x11d2, 0x94, 0x20, 0x0, 0x0, 0xf8, 0x8, 0x34, 0x60);

    // Use this guid in the SetOption to indicate if the tlbimporter should generate
    // the TCE adapters for COM connection point containers.
    ...
    // typelib import. {F17FF889-5A63-11d3-9FF2-00C04FF7431A}
    EXTERN_GUID(
    ...
    // CorLinkerOptions. {47E099B6-AE7C-4797-8317-B48AA645B8F9}
    EXTERN_GUID(MetaDataLinkerOptions, 0x47e099b6, 0xae7c, 0x4797, 0x83, 0x17, 0xb4, 0x8a, 0xa6, 0x45, 0xb8, 0xf9);

    // Use this guid in the SetOption to specify the runtime version stored in the
    // CLR metadata. {47E099B7-AE7C-4797-8317-B48AA645B8F9}
...
// <STRIP>The following definitions will get moved into <windows.h> by RTM but
// are kept here for the Alpha's and Beta's.</STRIP>
...
#define INTERFACE IMetaDataEmit
    DECLARE_INTERFACE_(IMetaDataEmit, IUnknown)
    {
        ...
        (mdTypeDef td,                  // [IN] the class/interface on which the event is being defined
         ...
         mdToken tkEventType,           // [IN] a reference (mdTypeRef or mdTypeRef) to the
                                        // Event class
         ...
         mdMethodDef rmdOtherMethods[], // [IN] optional array of other methods
        ...
        (                          // S_OK or error.
            ...
            DWORD dwCPlusTypeFlag, // [IN] Flag for the value type, selected
                                   // ELEMENT_TYPE_*
            void const *pValue,    // [IN] Constant value.
        ...
        (                              // S_OK or error.
            ...
            IMapToken * pHostMapToken, // [IN] Host IMapToken interface to receive
                                       // token remap notification
            IUnknown * pHandler) PURE; // [IN] An object to receive to receive error notification.

        STDMETHOD(MergeEnd)() PURE; // S_OK or error.

        // This interface is sealed.  Do not change, add, or remove anything. Instead,
        // derive a new iterface.
...
#define INTERFACE IMetaDataEmit2
    DECLARE_INTERFACE_(IMetaDataEmit2, IMetaDataEmit)
    {
        ...
        (                               // S_OK or error.
            ...
            mdToken rtkConstraints[],   // [IN] Array of type constraints
...
#define INTERFACE IMetaDataImport
    DECLARE_INTERFACE_(IMetaDataImport, IUnknown)
    {
        ...
        (HCORENUM * phEnum, mdTypeDef rTypeDefs[], ULONG cMax, ULONG * pcTypeDefs) PURE;
        STDMETHOD(EnumInterfaceImpls)
        (HCORENUM * phEnum, mdTypeDef td, mdInterfaceImpl rImpls[], ULONG cMax, ULONG * pcImpls) PURE;
        STDMETHOD(EnumTypeRefs)
        (HCORENUM * phEnum, mdTypeRef rTypeRefs[], ULONG cMax, ULONG * pcTypeRefs) PURE;
...
#define INTERFACE IMetaDataImport2
    DECLARE_INTERFACE_(IMetaDataImport2, IMetaDataImport)
    {
        ...
        (                             // S_OK or error.
            ...
            DWORD * pccBufSize) PURE; // [OUT] Size of the version string, wide chars,
...
#define INTERFACE IMetaDataConverter
    ...
    typedef struct
    {
        ...
        ULONG ulProcessor;       // [IN/OUT] Size of the Processor ID array/Actual # of
                                 // entries filled in.
...
#define INTERFACE IMetaDataAssemblyEmit
    DECLARE_INTERFACE_(IMetaDataAssemblyEmit, IUnknown)
    {
        ...
        (                                     // S_OK or error.
            ...
            mdToken tkImplementation,         // [IN] mdFile or mdAssemblyRef that provides
                                              // the resource.
            DWORD dwOffset,                   // [IN] Offset to the beginning of the resource within the
                                              // file.
...
#define INTERFACE IMetaDataAssemblyImport
    DECLARE_INTERFACE_(IMetaDataAssemblyImport, IUnknown)
    {
        ...
        (                                   // S_OK or error.
            ...
            ULONG cchName,                  // [IN] Size of buffer in wide chars.
            ULONG * pchName,                // [OUT] Actual # of wide chars in name.
            mdToken * ptkImplementation,    // [OUT] mdFile or mdAssemblyRef that
                                            // provides the ManifestResource.
        ...
        STDMETHOD_(void, CloseEnum)(HCORENUM hEnum) PURE; // Enum to be closed.
...
#define INTERFACE IMetaDataDispenserEx
    ...
    typedef enum
    {
        ...
        // generate a .reloc for a 25-bit PC relative address in an ia64 br.call
        // instruction
        srRelocIA64PcRel25,

        // generate a .reloc for a 64-bit PC relative address in an ia64 brl.call
        // instruction
        srRelocIA64PcRel64,
...
#define IMAGE_REL_BASED_IA64_PCREL60 12
    ...
    DECLARE_INTERFACE_(ICeeGen, IUnknown)
    {
        ...
        (HCEESECTION section, ULONG offset, HCEESECTION relativeTo, CeeSectionRelocType relocType) PURE;

        // use these only if you have special section requirements not handled
        // by other APIs
        STDMETHOD(GetSectionCreate)
        (const char *name, DWORD flags, HCEESECTION *section) PURE;

        STDMETHOD(GetSectionDataLen)(HCEESECTION section, ULONG * dataLen) PURE;

        STDMETHOD(GetSectionBlock)
        (HCEESECTION section, ULONG len, ULONG align = 1, void **ppBytes = 0) PURE;
    ...
    DECLARE_INTERFACE_(ICorModule, IUnknown)
    {
        ...
        STDMETHOD(GetCeeGen)(ICeeGen * *pCeeGen) PURE; // [OUT] cee generator

        STDMETHOD(GetMetaDataEmit)(IMetaDataEmit * *pEmitter) PURE; // [OUT] emitter
    };
    ...
    //--- IMetaDataTables
    //-------------------------------------
    // This API isn't big endian friendly since it indexes directly into the memory
    // that is stored in little endian format.
    ...
    DECLARE_INTERFACE_(IMetaDataTables, IUnknown)
    {
        ...
        STDMETHOD(GetNumTables)(ULONG * pcTables) PURE; // [OUT] Count of tables.
...
#define _META_DATA_META_CONSTANTS_DEFINED
    ...
    inline int IsRidType(ULONG ix)
    {
        return ix <= iRidMax;
    }
    inline int IsCodedTokenType(ULONG ix)
    {
        return (ix >= iCodedToken) && (ix <= iCodedTokenMax);
    }
    inline int IsRidOrToken(ULONG ix)
    {
        return ix <= iCodedTokenMax;
    }
    inline int IsHeapType(ULONG ix)
    {
        return ix >= iSTRING;
    }
    inline int IsFixedType(ULONG ix)
    {
        return (ix < iSTRING) && (ix > iCodedTokenMax);
    }
#endif
#endif
    ...
    DECLARE_INTERFACE_(INativeImageDependency, IUnknown)
    {
        ...
        (CORCOMPILE_NGEN_SIGNATURE * pNativeSign // [OUT] INVALID_NGEN_SIGNATURE if
                                                 // there is no hard-bound dependancy
         ) PURE;
    };
    ...
    DECLARE_INTERFACE_(INativeImageInstallInfo, IUnknown)
    {
        ...
        (LPWSTR szConfigString, // [OUT]
         DWORD * pdwLength      // [IN|OUT] - Number of WCHARs written including
                                // terminating NULL
         ) PURE;
        ...
        // Sets *pdwBufferSize and returns ERROR_INSUFFICIENT_BUFFER for insufficient
        // ppbBuffer
    ...
    // NativeAssembly can be used. The pUnkBindSink argument of
    // CAssemblyName::BindToObject() can be queried for this interface
    //
    //*****************************************************************************
...
#include <pshpack1.h>
    typedef struct
    {
        ...
        mdMemberRef m_entryPoint; // member ref token giving entry point, format is
                                  // lib:entrypoint
    } COR_NATIVE_LINK;
#include <poppack.h>
...
#define COR_BASE_SECURITY_ATTRIBUTE_CLASS_ANSI "System.Security.Permissions.SecurityAttribute"
    ...
    //
    // Name of custom attribute used to indicate that per-call security checks
    // should be disabled for P/Invoke calls.
    //
...
// Return true if element type is a modifier, i.e. ELEMENT_TYPE_MODIFIER bits
// are turned on. For now, it is checking for ELEMENT_TYPE_PTR and
// ELEMENT_TYPE_BYREF as well. This will be removed when we turn on
// ELEMENT_TYPE_MODIFIER bits for these two enum members.
//
...
inline ULONG CorSigUncompressedDataSize(PCCOR_SIGNATURE pData)
{
    if ((*pData & 0x80) == 0)
        return 1;
...
// Return value is the number of bytes that the integer occupies in the
// compressed format It is caller's responsibility to ensure pDataOut has at
// least 4 bytes to be written to.
//
// This function returns -1 if pass in with an incorrectly compressed data, such
// as
// (*pBytes & 0xE0) == 0XE0.
/////////////////////////////////////////////////////////////////////////////////////////////
inline ULONG CorSigUncompressBigData(PCCOR_SIGNATURE &pData) // [IN,OUT] compressed data
{
    ULONG res;
...
FORCEINLINE ULONG CorSigUncompressData(PCCOR_SIGNATURE &pData) // [IN,OUT] compressed data
{
...
inline ULONG CorSigUncompressData( // return number of bytes of that compressed
                                   // data occupied in pData
...
inline ULONG CorSigUncompressToken( // return number of bytes of that compressed
                                    // data occupied in pData
    PCCOR_SIGNATURE pData,          // [IN] compressed data
...
FORCEINLINE ULONG CorSigUncompressCallingConv(PCCOR_SIGNATURE &pData) // [IN,OUT] compressed data
{
    return *pData++;
}
...
inline ULONG CorSigUncompressSignedInt( // return number of bytes of that
                                        // compressed data occupied in pData
    PCCOR_SIGNATURE pData,              // [IN] compressed data
...
inline ULONG CorSigUncompressElementType( // return number of bytes of that
                                          // compressed data occupied in pData
    PCCOR_SIGNATURE pData,                // [IN] compressed data
...
/////////////////////////////////////////////////////////////////////////////////////////////
//
// Given an uncompressed unsigned integer (iLen), Store it to pDataOut in a
// compressed format. Return value is the number of bytes that the integer
// occupies in the compressed format. It is caller's responsibilityt to ensure
// *pDataOut has at least 4 bytes to write to.
//
// Note that this function returns -1 if iLen is too big to be compressed. We
// currently can only represent to 0x1FFFFFFF.
//
/////////////////////////////////////////////////////////////////////////////////////////////
inline ULONG CorSigCompressData( // return number of bytes that compressed form
                                 // of iLen will take
...
// compress a token
// The least significant bit of the first compress byte will indicate the token
// type.
//
inline ULONG CorSigCompressToken( // return number of bytes that compressed form
                                  // of iLen will take
...
//
inline ULONG CorSigCompressSignedInt( // return number of bytes that compressed
                                      // form of iData will take
...
// uncompress encoded element type
inline ULONG CorSigCompressElementType( // return number of bytes of that
                                        // compressed data occupied in pData
...
// signatures).
inline ULONG CorSigCompressPointer( // return number of bytes of that compressed
                                    // data occupied
...
inline ULONG CorSigUncompressPointer( // return number of bytes of that
                                      // compressed data occupied
    PCCOR_SIGNATURE pData,            // [IN] compressed data
