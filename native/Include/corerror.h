...
#define MSEE_E_MULTCOPIESLOADED                                                                                        \
    EMAKEHR(0x1012) // Multiple copies of MSCOREE.dll have been loaded by the same
                    // process.
...
#define COR_E_FIXUPSINEXE                                                                                              \
    EMAKEHR(0x1019) // Attempt to load an unverifiable exe with fixups (IAT with
                    // more than 2 sections or a TLS section)
#define COR_E_NO_LOADLIBRARY_ALLOWED                                                                                   \
    EMAKEHR(0x101A) // Attempt to LoadLibrary a managed image in an improper way
                    // (only assemblies with EAT's area allowed.)
#define COR_E_NEWER_RUNTIME                                                                                            \
    EMAKEHR(0x101B) // The assembly is built by a runtime newer than the currently
...
#define FUSION_E_INVALID_PRIVATE_ASM_LOCATION                                                                          \
    EMAKEHR(0x1041)                                      // The private assembly was located outside the appbase
                                                         // directory.
...
#define FUSION_E_DATABASE_ERROR                                                                                        \
    EMAKEHR(0x1046)                           // An unexpected error was encountered in the Assembly Cache
                                              // database.
#define FUSION_E_INVALID_NAME EMAKEHR(0x1047) // The given assembly name or codebase was invalid.
#define FUSION_E_CODE_DOWNLOAD_DISABLED                                                                                \
    EMAKEHR(0x1048)                                   // HTTP download of assemblies has been disabled for this
                                                      // appdomain.
#define FUSION_E_UNINSTALL_DISALLOWED EMAKEHR(0x1049) // Uninstall of given assembly is not allowed.
#define FUSION_E_HOST_GAC_ASM_MISMATCH                                                                                 \
    EMAKEHR(0x1050) // Assembly in host store has a different signature than
                    // assembly in GAC
...
#define CLDB_E_SMDUPLICATE                                                                                             \
    EMAKEHR(0x110A)                         // Create of shared memory failed.  A memory mapping of the
...
#define TLBX_E_BAD_MSCOREE_TLB                                                                                         \
    EMAKEHR(0x1169)                                // Typelib export: can't get a required typeinfo from
                                                   // mscoree.tlb.
...
#define TLBX_E_INVALIDLCIDPARAM                                                                                        \
    EMAKEHR(0x1176) // Typelib export: Encounterd an LCID attribute set to an
                    // invalid param.
#define TLBX_E_LCIDONDISPONLYITF                                                                                       \
    EMAKEHR(0x1177)                             // Typelib export: Encounterd an LCID attribute on a disp only
                                                // interface.
...
#define TLBX_I_REF_TYPE_AS_STRUCT                                                                                      \
    SMAKEHR(0x117C)                                  // Typelib export: referenct tyep had layout, exported as
                                                     // struct.
...
#define META_E_TYPEDEF_MISMATCH                                                                                        \
    EMAKEHR(0x1182)                            // Merge: an import typedef matched ns.name, but not version
                                               // and guid.
#define META_E_MERGE_COLLISION EMAKEHR(0x1183) // Merge: conflict between import and emit
#define TLBX_W_NON_INTEGRAL_CA_TYPE                                                                                    \
    SMAKEHR(0x1184) // TypeLib import: Ignoring IDL custom attribute -- does not
                    // have an integral value.
#define TLBX_W_IENUM_CA_ON_IUNK                                                                                        \
    SMAKEHR(0x1185)                                 // TypeLib import: Ignoring IDL custom attribute -- using
                                                    // IEnum CA on an IUnknown derived interface.
...
#define META_E_INTFCEIMPL_NOT_FOUND                                                                                    \
    EMAKEHR(0x118B)                                     // Merge: Class already in emit scope, but interfaceimpl not
                                                        // found
#define TLBX_E_NO_CRITICALHANDLE_ARRAYS EMAKEHR(0x118C) // TypeLib export: detected array of CriticalHandles
#define META_E_CLASS_LAYOUT_INCONSISTENT                                                                               \
    EMAKEHR(0x118D) // Merge: Class is duplicated but class layout information is
                    // not consistent
#define META_E_FIELD_MARSHAL_NOT_FOUND                                                                                 \
    EMAKEHR(0x118E)                                // Merge: Field is duplicated but we cannot find the matching
                                                   // FieldMarshal information
#define META_E_METHODSEM_NOT_FOUND EMAKEHR(0x118F) // Merge:

#define META_E_EVENT_NOT_FOUND                                                                                         \
    EMAKEHR(0x1190) // Merge: Method is duplicated but we cannot find the matching
                    // event info.
#define META_E_PROP_NOT_FOUND                                                                                          \
    EMAKEHR(0x1191)                                   // Merge: Method is duplicated but we cannot find the maching
                                                      // property info.
...
#define META_S_DUPLICATE                                                                                               \
    SMAKEHR(0x1197)                                // Attempt to define an object that already exists in valid
                                                   // scenerios.
...
#define META_E_GENERICPARAM_INCONSISTENT                                                                               \
    EMAKEHR(0x119C)                            // Merge: duplicated types/methods have inconsistent
                                               // GenericParams
...
#define META_E_TYPEDEF_MISSING                                                                                         \
    EMAKEHR(0x119F) // Merge: An input scope has a TypeRef which should but
                    // doesn't have a matching TypeDef.
...
#define TLBX_E_NULL_MODULE                                                                                             \
    EMAKEHR(0x11A2)                            // TypeLib export: the hMod of a loaded class is 0; can't
                                               // export it.
...
#define TLBX_E_CRM_INVALID_SIG                                                                                         \
    EMAKEHR(0x11A9)                                 // TypeLib export: the specified COM register method does not
...
#define TLBX_E_INVALID_NAMESPACE                                                                                       \
    EMAKEHR(0x11AD)                         // TypeLib import: an imported typelib has an invalid
                                            // namespace name.
...
#define TLBX_E_NONVISIBLEVALUECLASS                                                                                    \
    EMAKEHR(0x11B0) // Typelib export: Non COM visible value type in method
                    // signature.
#define TLBX_E_LPTSTR_NOT_ALLOWED                                                                                      \
    EMAKEHR(0x11B1) // Typelib export: Types which contain the native type
                    // NATIVE_TYPE_LPTSTR are not allowed to be exported to COM.
#define TLBX_E_AUTO_CS_NOT_ALLOWED                                                                                     \
    EMAKEHR(0x11B2) // Typelib export: Types with a char set of auto are not
                    // allowed to be exported to COM.
#define TLBX_S_NOSTDINTERFACE                                                                                          \
    SMAKEHR(0x11B3)                               // Typelib export: Found an interface marked as IID_IDispatch
                                                  // or IID_IUnknown.
...
#define TLBX_E_PARAM_ERROR_UNNAMED                                                                                     \
    EMAKEHR(0x11B9)                              // Typelib import: param type couldn't be converted -- param
                                                 // name unknown.
...
#define TLBX_E_BAD_SIGNATURE                                                                                           \
    EMAKEHR(0x11BD)                                 // Typelib export: unconvertable signature (use specific error
                                                    // for reporting!)
...
#define META_E_CA_INVALID_ARG_FOR_TYPE                                                                                 \
    EMAKEHR(0x11C9)                            // Known attribute parser found an argument that is invalid
                                               // for the object it is applied to.
#define META_E_CA_INVALID_UUID EMAKEHR(0x11CA) // The format of the UUID was invalid.
#define META_E_CA_INVALID_MARSHALAS_FIELDS                                                                             \
    EMAKEHR(0x11CB)                                   // The MarshalAs attribute has fields set that are not valid
                                                      // for the specified unmanaged type.
...
#define TLBX_W_NO_PROPS_IN_EVENTS                                                                                      \
    EMAKEHR(0x11D3)                                     // TypeLib import: Detected properties in a source
                                                        // dispinterface.
...
#define TLBX_W_BAD_SAFEARRAYFIELD_NO_ELEMENTVT                                                                         \
    SMAKEHR(0x11DE) // TypeLib export: System.Array SAFEARRAY field without a
                    // SafeArraySubType.
#define TLBX_W_LAYOUTCLASS_AS_INTERFACE                                                                                \
    SMAKEHR(0x11DF)                              // TypeLib export: Class with layout parameter of field marked
...
#define VLDTR_E_TD_NOTIFACEOBJEXTNULL                                                                                  \
    EMAKEHR(0x1210)                                  // TypeDef that's not an Interface and not System.Object
                                                     // extends nil parent.
...
#define VLDTR_E_MR_NOTVARARG                                                                                           \
    EMAKEHR(0x1225)                          // MemberRef has Method parent but calling convention is not
                                             // VARARG.
...
#define VLDTR_E_FL_TDNOTEXPLCT                                                                                         \
    EMAKEHR(0x122F)                             // FieldLayout2 parent TypeDef is not marked with
                                                // ExplicitLayout.
...
#define VLDTR_E_TD_EXTTRRES                                                                                            \
    EMAKEHR(0x1236)                               // TypeDef extends a TypeRef which resolves to a TypeDef in
                                                  // the same module.
...
#define VLDTR_E_MD_THISSTATIC                                                                                          \
    EMAKEHR(0x123A)                                    // Method is marked static but has HASTHIS/EXPLICITTHIS set on
...
#define VLDTR_E_IFACE_BADIFACE                                                                                         \
    EMAKEHR(0x12A8)                                     // In InterfaceImpl, the implemented token is not TypeDef or
                                                        // TypeRef
...
#define VLDTR_E_GP_UNEXPECTED_OWNER_FOR_VARIANT_VAR                                                                    \
    EMAKEHR(0x1B2C) // GenericParam has variance but its owner is not an interface
                    // or delegate
#define VLDTR_E_GP_ILLEGAL_VARIANT_MVAR                                                                                \
    EMAKEHR(0x1B2D)                                       // GenericParam is a method type parameter and must be
                                                          // non-variant
#define VLDTR_E_GP_ILLEGAL_VARIANCE_FLAGS EMAKEHR(0x1B2E) // GenericParam has illegal value for variance flags
#define VLDTR_E_GP_REFANDVALUETYPE                                                                                     \
...
#define VLDTR_E_GPC_NONCONTIGUOUS                                                                                      \
    EMAKEHR(0x1B32)                               // GenericParamConstraint is non-contiguous with preceeding
                                                  // constraints for same owner
...
#define VLDTR_E_MS_ARITYMISMATCH                                                                                       \
    EMAKEHR(0x1B38)                                 // MethodSpec arity of generic method and instantiation do not
                                                    // match
...
#define VLDTR_E_SIG_ARITYMISMATCH                                                                                      \
    EMAKEHR(0x1B3B)                               // Signature has generic type of arity instantiated at
                                                  // different arity
...
#define VLDTR_E_MD_GENERIC_BADCALLCONV                                                                                 \
    EMAKEHR(0x1B3F)                               // Method cannot be both generic and have non-default calling
                                                  // convention
...
#define VLDTR_E_EP_GENERIC_TYPE                                                                                        \
    EMAKEHR(0x1B47)                               // Entry point in CLR header is the token for a method in a
                                                  // generic type
...
#define VLDTR_E_MI_ARITYMISMATCH                                                                                       \
    EMAKEHR(0x1B4A)                                    // MethodImpl overrides generic method of arity n with generic
                                                       // method of arity m
...
#define VLDTR_E_SIG_BADTOKTYPE                                                                                         \
    EMAKEHR(0x1B51)                                      // Signature has token following ELEMENT_TYPE_CLASS
...
#define CORDBG_E_REMOTE_CONNECTION_FATAL_ERROR                                                                         \
    EMAKEHR(0x1322) // Generic error that the device connection has been broken
...
// we're trying to EnC.
#define CORDBG_E_ENC_CANT_ADD_FIELD_TO_VALUE_OR_LAYOUT_CLASS                                                           \
    EMAKEHR(0x1338) // Adding a field to a value or layout class is prohibitted,
...
#define CORDBG_E_ENC_INTERNAL_ERROR                                                                                    \
    EMAKEHR(0x1341) // Generic message for "Something user doesn't control went
                    // wrong" message.
#define CORDBG_E_ENC_HANGING_FIELD                                                                                     \
    EMAKEHR(0x1342) // The field was added via enc after the class was loaded, and
                    // so instead of
...
#define CORDBG_E_DEBUGGING_NOT_POSSIBLE                                                                                \
    EMAKEHR(0x1346) // Debugging isn't possible due to an incompatability within
                    // the CLR implementation.
#define CORDBG_E_KERNEL_DEBUGGER_ENABLED                                                                               \
    EMAKEHR(0x1347) // Debugging isn't possible because a kernel debugger is
                    // enabled on the system.
#define CORDBG_E_KERNEL_DEBUGGER_PRESENT                                                                               \
    EMAKEHR(0x1348) // Debugging isn't possible because a kernel debugger is
...
// NOTE!  YOU CANNOT PUT MORE ERRORS HERE!  They run into the range for
// profiling errors.  All new new error need to be added below (search for
// cordbg_e_ in your editor).
...
#define CORPROF_E_NOT_YET_AVAILABLE                                                                                    \
    EMAKEHR(0x135B) // This is a general error used to indicated that the
                    // information
// requested is not yet available
#define CORPROF_E_TYPE_IS_PARAMETERIZED                                                                                \
    EMAKEHR(0x135C) // The given type is a generic and cannot be used with this
                    // method.
#define CORPROF_E_FUNCTION_IS_PARAMETERIZED                                                                            \
    EMAKEHR(0x135D) // The given function is a generic and cannot be used with
                    // this method.

//**** Security errors
#define SECURITY_E_XML_TO_ASN_ENCODING EMAKEHR(0x1400) // Failed to convert XML to ASN
#define SECURITY_E_INCOMPATIBLE_SHARE                                                                                  \
    EMAKEHR(0x1401)                             // Loading this assembly would produce a different grant set
                                                // from other instances
#define SECURITY_E_UNVERIFIABLE EMAKEHR(0x1402) // Unverifable code failed policy check
#define SECURITY_E_INCOMPATIBLE_EVIDENCE                                                                               \
    EMAKEHR(0x1403) // Assembly already loaded without additional security evidence.
...
// MessageText:
//
// A program contained invalid IL or bad metadata.  Usually this is a compiler
// bug.
//
...
// MessageText:
//
//  Thrown if value (a floating point number) is either the not a number value
//  (NaN) or +- infinity value VB needs this stuff
...
// MessageText:
//
// Is raised by the EE when the execution stack overflows as it is attempting to
// ex
//
...
// MessageText:
//
// Wait(), Notify() or NotifyAll() was called from an unsynchronized ** block of
// c
//
...
// MessageText:
//
// Thrown into a thread to cause it to stop. This exception is typically not
// caught
//
...
// MessageText:
//
// Adding the given count to the semaphore would cause it to exceed its maximum
// count.
//
...
// MessageText:
//
// - If you attempt to invoke a non-static method with a null Object - If you
// atte
//
...
//            Range 0x1B00 - 0x1BFF is reserved for MD Validator errors (see
//            above VLDTR_E_...)
//

//**** More debugger error 1C00 - 1CFF
//
#define CORDBG_E_THREAD_NOT_SCHEDULED                                                                                  \
    EMAKEHR(0x1C00)                                         // Thread is not scheduled. Thus we may not have OSThreadId,
                                                            // handle, or context
...
#define CORDBG_E_CANT_UNWIND_ABOVE_CALLBACK                                                                            \
    EMAKEHR(0x1C03) // When intercepting an exception, cannot intercept above the
                    // current frame.
#define CORDBG_E_INTERCEPT_FRAME_ALREADY_SET                                                                           \
    EMAKEHR(0x1C04) // The intercept frame for this exception has already been set.
...
#define CORDBG_E_ENC_NESTED_HANLDERS                                                                                   \
    EMAKEHR(0x1C0B)                                     // Frame cant be updated due to change in max nesting of
                                                        // handlers
...
#define CORDBG_E_FEABORT_DELAYED_UNTIL_THREAD_RESUMED                                                                  \
    EMAKEHR(0x1C0F) // Attempt to func eval abort on a suspended thread.
#define CORDBG_E_NOTREADY                                                                                              \
    EMAKEHR(0x1C10) // The LS is not in a good spot to perform the requested
                    // operation.
#define CORDBG_E_CANNOT_RESOLVE_ASSEMBLY                                                                               \
    EMAKEHR(0x1c11) // We failed to resolve assembly given an AssemblyRef token.
                    // Assembly may be not loaded yet or not a valid token.
#define CORDBG_E_MUST_BE_IN_LOAD_MODULE                                                                                \
    EMAKEHR(0x1C12) // Must be in context of LoadModule callback to perform
                    // requested operation
#define CORDBG_E_CANNOT_BE_ON_ATTACH                                                                                   \
    EMAKEHR(0x1C13)                                     // Requested operation cannot be performed during an attach
                                                        // operation
...
#define CORDBG_E_CANNOT_DEBUG_FIBER_PROCESS                                                                            \
    EMAKEHR(0x1C16) // For Whidbey, we don't support debugging fiber mode managed
                    // process
#define CORDBG_E_MUST_BE_IN_CREATE_PROCESS                                                                             \
    EMAKEHR(0x1C17) // Must be in context of CreateProcess callback to perform
                    // requested operation
#define CORDBG_E_DETACH_FAILED_OUTSTANDING_EVALS                                                                       \
    EMAKEHR(0x1C18) // All outstanding func-evals have not completed, detaching is
