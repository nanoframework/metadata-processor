...
template <DWORD SIZE, DWORD INCREMENT> HRESULT CQuickMemoryBase<SIZE, INCREMENT>::ReSizeNoThrow(SIZE_T iItems)
{
#ifdef _BLD_CLR
#ifdef _DEBUG
 ...
 *
 *************************************************************************************/
#define CHECK_REMAINDER                                                                                                \
    if (cbTotal >= cbTotalMax)                                                                                         \
    {                                                                                                                  \
        hr = E_FAIL;                                                                                                   \
        goto ErrExit;                                                                                                  \
    }
    ...
    ULONG *pcbTotal) // Initially, *pcbTotal contains the remaining size of the sig blob
{
    ...
    switch (ulElementType)
    {
        ...
        case ELEMENT_TYPE_CMOD_OPT:
            ...
            if (ulElementType == ELEMENT_TYPE_CMOD_REQD || ulElementType == ELEMENT_TYPE_CMOD_OPT)
            {
                // skip over base type
                CHECK_REMAINDER;
                cb = cbTotalMax - cbTotal;
...
extern "C"
{
        ...
        *__stdcall SectEH_EHClause(void *pSectEH, unsigned idx, IMAGE_COR_ILMETHOD_SECT_EH_CLAUSE_FAT *buff)
    {
        ...
        COR_ILMETHOD_SECT_EH_CLAUSE_SMALL *smallClause =
            (COR_ILMETHOD_SECT_EH_CLAUSE_SMALL *)&((COR_ILMETHOD_SECT_EH *)pSectEH)->Small.Clauses[idx];
    ...
    unsigned __stdcall SectEH_SizeExact(unsigned ehCount, IMAGE_COR_ILMETHOD_SECT_EH_CLAUSE_FAT *clauses)
    {
        ...
        for (unsigned i = 0; i < ehCount; i++)
        {
            ...
            if (fatClause->GetTryOffset() > 0xFFFF || fatClause->GetTryLength() > 0xFF ||
                fatClause->GetHandlerOffset() > 0xFFFF || fatClause->GetHandlerLength() > 0xFF)
            {
                return (COR_ILMETHOD_SECT_EH_FAT::Size(ehCount));
            }
        }
        ...
        ULONG *ehTypeOffsets)
    {
        ...
        if (COR_ILMETHOD_SECT_EH_SMALL::Size(ehCount) < COR_ILMETHOD_SECT_SMALL_MAX_DATASIZE)
        {
            ...
            for (i = 0; i < ehCount; i++)
            {
                ...
                COR_ILMETHOD_SECT_EH_CLAUSE_SMALL *smallClause =
                    (COR_ILMETHOD_SECT_EH_CLAUSE_SMALL *)&EHSect->Clauses[i];
