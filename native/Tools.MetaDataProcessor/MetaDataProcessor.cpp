...
#define PARAM_STRING(val, parm1Name, parm1Desc)                                                                        \
    param = new CLR_RT_ParseOptions::Parameter_String(val, parm1Name, parm1Desc);                                      \
    cmd->m_params.push_back(param)
#define PARAM_BOOLEAN(val, parm1Name, parm1Desc)                                                                       \
    param = new CLR_RT_ParseOptions::Parameter_Boolean(val, parm1Name, parm1Desc);                                     \
    cmd->m_params.push_back(param)
#define PARAM_INTEGER(val, parm1Name, parm1Desc)                                                                       \
    param = new CLR_RT_ParseOptions::Parameter_Integer(val, parm1Name, parm1Desc);                                     \
    cmd->m_params.push_back(param)
#define PARAM_FLOAT(val, parm1Name, parm1Desc)                                                                         \
    param = new CLR_RT_ParseOptions::Parameter_Float(val, parm1Name, parm1Desc);                                       \
    cmd->m_params.push_back(param)
...
#define OPTION_GENERIC(optName, optDesc)                                                                               \
    cmd = new CLR_RT_ParseOptions::Command(optName, optDesc);                                                          \
    m_commands.push_back(cmd)
#define OPTION_SET(val, optName, optDesc)                                                                              \
    cmd = new CLR_RT_ParseOptions::Command_SetFlag(val, optName, optDesc);                                             \
    m_commands.push_back(cmd)
#define OPTION_CALL(fpn, optName, optDesc)                                                                             \
    cmd = new Command_Call(*this, &Settings::fpn, optName, optDesc);                                                   \
    m_commands.push_back(cmd)

#define OPTION_STRING(val, optName, optDesc, parm1Name, parm1Desc)                                                     \
    OPTION_GENERIC(optName, optDesc);                                                                                  \
    PARAM_STRING(val, parm1Name, parm1Desc)
#define OPTION_BOOLEAN(val, optName, optDesc, parm1Name, parm1Desc)                                                    \
    OPTION_GENERIC(optName, optDesc);                                                                                  \
    PARAM_BOOLEAN(val, parm1Name, parm1Desc)
#define OPTION_INTEGER(val, optName, optDesc, parm1Name, parm1Desc)                                                    \
    OPTION_GENERIC(optName, optDesc);                                                                                  \
    PARAM_INTEGER(val, parm1Name, parm1Desc)
#define OPTION_FLOAT(val, optName, optDesc, parm1Name, parm1Desc)                                                      \
    OPTION_GENERIC(optName, optDesc);                                                                                  \
    PARAM_FLOAT(val, parm1Name, parm1Desc)
    ...
    HRESULT Cmd_LoadHints(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        ...
        NANOCLR_CHECK_HRESULT(
            metaDataCollention.LoadHints(PARAM_EXTRACT_STRING(params, 0), PARAM_EXTRACT_STRING(params, 1)));

        NANOCLR_NOCLEANUP();
    }

    HRESULT
    Cmd_IgnoreAssembly(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        NANOCLR_HEADER();
    ...
    HRESULT Cmd_Parse(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        ...
        if (!metaDataParser)
            NANOCLR_CHECK_HRESULT(metaDataCollention.CreateAssembly(metaDataParser));
    ...
    HRESULT
    Cmd_VerboseMinimize(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        NANOCLR_HEADER();
    ...
    HRESULT
    Cmd_ExcludeClassByName(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        NANOCLR_HEADER();
    ...
    HRESULT
    Cmd_GenerateStringsTable(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        NANOCLR_HEADER();
    ...
    HRESULT
    Cmd_ImportResource(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        NANOCLR_HEADER();
    ...
    HRESULT Cmd_Compile(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        ...
        if (!metaDataParser)
            NANOCLR_SET_AND_LEAVE(E_FAIL);

        metaDataParser->m_resources = resources;
        resources.clear();

        {
            ...
            NANOCLR_CHECK_HRESULT(
                CLR_RT_FileStore::SaveFile(szFile.c_str(), (CLR_UINT8 *)buf.Ptr(), (DWORD)buf.Size()));
    ...
    HRESULT
    Cmd_GenerateSkeleton(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        NANOCLR_HEADER();
    ...
    HRESULT
    Cmd_RefreshAssembly(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        NANOCLR_HEADER();
        ...
        if (currentAssembly)
        {
            ...
            NANOCLR_CHECK_HRESULT(
                CLR_RT_FileStore::SaveFile(szFile, (CLR_UINT8 *)currentAssembly->m_header, (DWORD)len));
        }
    ...
    HRESULT Cmd_GenerateDependency__OutputAssembly(
        CLR_XmlUtil xml,
        IXMLDOMNode *node,
        ...
        CLR_RT_Assembly *assm)
    {
        ...
        swprintf(
            ...
            ARRAYSIZE(rgBuffer),
            L"%d.%d.%d.%d",
            assm->m_header->version.iMajorVersion,
            assm->m_header->version.iMinorVersion,
            assm->m_header->version.iBuildNumber,
            assm->m_header->version.iRevisionNumber);
        ...
        NANOCLR_CHECK_HRESULT(xml.PutAttribute(
            NULL,
            L"Hash",
            WatchAssemblyBuilder::ToHex(assm->ComputeAssemblyHash()),
            fFound,
            assmNode));
        NANOCLR_CHECK_HRESULT(
            xml.PutAttribute(NULL, L"Flags", WatchAssemblyBuilder::ToHex(assm->m_header->flags), fFound, assmNode));

        NANOCLR_NOCLEANUP();
    }

    HRESULT
    Cmd_GenerateDependency(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        NANOCLR_HEADER();
        ...
        NANOCLR_FOREACH_ASSEMBLY(g_CLR_RT_TypeSystem)
        {
...

            {
                ...
                for (int i = 0; i < pASSM->m_pTablesSize[TBL_AssemblyRef]; i++, src++)
                {
                    ...
                    CLR_RT_Assembly *assmRef =
                        g_CLR_RT_TypeSystem.FindAssembly(pASSM->GetString(src->name), &src->version, true);
                    if (!assmRef)
                        NANOCLR_SET_AND_LEAVE(CLR_E_NULL_REFERENCE);

                    NANOCLR_CHECK_HRESULT(
                        Cmd_GenerateDependency__OutputAssembly(xml, assmNode, assmRefNode, L"AssemblyRef", assmRef));
                }
            }

            {
                ...
                for (int i = 0; i < pASSM->m_pTablesSize[TBL_TypeDef]; i++, src++, dst++)
                {
                    ...
                    NANOCLR_CHECK_HRESULT(
                        xml.PutAttribute(NULL, L"Hash", WatchAssemblyBuilder::ToHex(dst->m_hash), fFound, typeNode));
                }
            }
        }
        NANOCLR_FOREACH_ASSEMBLY_END();
    ...
    HRESULT
    Cmd_CreateDatabase(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        NANOCLR_HEADER();
    ...
    void BuildOptions()
    {
        ...
        OPTION_INTEGER(
            &s_CLR_RT_fTrace_RedirectLinesPerFile,
            L"-Trace_RedirectLinesPerFile",
            L"",
            L"<lines>",
            L"Lines per File");
        ...
        OPTION_STRING(
            &patchNative,
            L"-patchNative",
            L"ARM code to include in the patch",
            L"<file>",
            L"Native code file");
        ...
        OPTION_CALL(
            Cmd_DumpExports,
            L"-dump_exports",
            L"Generates a report of an assembly's metadata, more readable format");
        PARAM_GENERIC(L"<file>", L"Report file");

        OPTION_CALL(
            Cmd_GenerateSkeleton,
            L"-generate_skeleton",
            L"Generates a skeleton for the methods implemented in native code");
        ...
        OPTION_INTEGER(
            &s_CLR_RT_fJitter_Trace_Statistics,
            L"-Jitter_Trace_Statistics",
            L"",
            L"<level>",
            L"Level of verbosity");
        OPTION_INTEGER(
            &s_CLR_RT_fJitter_Trace_Compile,
            L"-Jitter_Trace_Compile",
            L"",
            L"<level>",
            L"Level of verbosity");
#endif
        ...
        OPTION_CALL(
            Cmd_GenerateDependency,
            L"-generate_dependency",
            L"Generate an XML file with the relationship between assemblies");
        PARAM_GENERIC(L"<file>", L"Output file");
...
int _tmain(int argc, _TCHAR *argv[])
{
    ...
    // get version of executing process (that's metadataprocessor)
    ...
    GetFileVersionInfoA(exe, dwHandle, sz, &buf[0]);
    VS_FIXEDFILEINFO *pvi;
    sz = sizeof(VS_FIXEDFILEINFO);
    VerQueryValueA(&buf[0], "\\", (LPVOID *)&pvi, (unsigned int *)&sz);
    ////
    ...
    wprintf(METADATAPROCESSOR_COPYRIGHT_STRING);
    wprintf(L"For documentation, report issues and support visit our GitHub "
            L"repo: www.GitHub.com\\nanoFramework\r\n\r\n");
    ...
    if (FAILED(hr))
    {
        ErrorReporting::Print(
            ...
            0,
            L"%S (%S)",
            CLR_RT_DUMP::GETERRORMESSAGE(hr),
            CLR_RT_DUMP::GETERRORDETAIL());
        fflush(stdout);
    }
