...
void CLR_RT_Assembly::Dump_CloseDevice(FILE *&outputDeviceFile)
{
    if (outputDeviceFile)
    {
        fclose(outputDeviceFile);
        outputDeviceFile = NULL;
    }
}
...
void CLR_RT_Assembly::Dump(bool fNoByteCode)
{
    ...
    Dump_Printf(
        "Size: AssemblyRef     %6d byte(s) %6d item(s) [Runtime: %6d]\n",
        m_pTablesSize[TBL_AssemblyRef] * sizeof(CLR_RECORD_ASSEMBLYREF),
        m_pTablesSize[TBL_AssemblyRef],
        m_pTablesSize[TBL_AssemblyRef] * sizeof(CLR_RT_AssemblyRef_CrossReference));
    Dump_Printf(
        "Size: TypeRef         %6d byte(s) %6d item(s) [Runtime: %6d]\n",
        m_pTablesSize[TBL_TypeRef] * sizeof(CLR_RECORD_TYPEREF),
        m_pTablesSize[TBL_TypeRef],
        m_pTablesSize[TBL_TypeRef] * sizeof(CLR_RT_TypeRef_CrossReference));
    Dump_Printf(
        "Size: FieldRef        %6d byte(s) %6d item(s) [Runtime: %6d]\n",
        m_pTablesSize[TBL_FieldRef] * sizeof(CLR_RECORD_FIELDREF),
        m_pTablesSize[TBL_FieldRef],
        m_pTablesSize[TBL_FieldRef] * sizeof(CLR_RT_FieldRef_CrossReference));
    Dump_Printf(
        "Size: MethodRef       %6d byte(s) %6d item(s) [Runtime: %6d]\n",
        m_pTablesSize[TBL_MethodRef] * sizeof(CLR_RECORD_METHODREF),
        m_pTablesSize[TBL_MethodRef],
        m_pTablesSize[TBL_MethodRef] * sizeof(CLR_RT_MethodRef_CrossReference));
    Dump_Printf(
        "Size: TypeDef         %6d byte(s) %6d item(s) [Runtime: %6d]\n",
        m_pTablesSize[TBL_TypeDef] * sizeof(CLR_RECORD_TYPEDEF),
        m_pTablesSize[TBL_TypeDef],
        m_pTablesSize[TBL_TypeDef] * sizeof(CLR_RT_TypeDef_CrossReference));
    Dump_Printf(
        "Size: FieldDef        %6d byte(s) %6d item(s) [Runtime: %6d]\n",
        m_pTablesSize[TBL_FieldDef] * sizeof(CLR_RECORD_FIELDDEF),
        m_pTablesSize[TBL_FieldDef],
        m_pTablesSize[TBL_FieldDef] * sizeof(CLR_RT_FieldDef_CrossReference));
    Dump_Printf(
        "Size: MethodDef       %6d byte(s) %6d item(s) [Runtime: %6d]\n",
        m_pTablesSize[TBL_MethodDef] * sizeof(CLR_RECORD_METHODDEF),
        m_pTablesSize[TBL_MethodDef],
        m_pTablesSize[TBL_MethodDef] * sizeof(CLR_RT_MethodDef_CrossReference));
    Dump_Printf(
        "Size: Attributes      %6d byte(s) %6d item(s)\n",
        m_pTablesSize[TBL_Attributes] * sizeof(CLR_RECORD_ATTRIBUTE),
        m_pTablesSize[TBL_Attributes]);
    Dump_Printf(
        "Size: TypeSpec        %6d byte(s) %6d item(s)\n",
        m_pTablesSize[TBL_TypeSpec] * sizeof(CLR_RECORD_TYPESPEC),
        m_pTablesSize[TBL_TypeSpec]);
    Dump_Printf(
        "Size: Resources Files %6d byte(s) %6d item(s)\n",
        m_pTablesSize[TBL_ResourcesFiles] * sizeof(CLR_RECORD_RESOURCE_FILE),
        m_pTablesSize[TBL_ResourcesFiles]);
    Dump_Printf(
        "Size: Resources       %6d byte(s) %6d item(s)\n",
        m_pTablesSize[TBL_Resources] * sizeof(CLR_RECORD_RESOURCE),
        m_pTablesSize[TBL_Resources]);
...
#define ASMOFF(x) (UINT32)((size_t)x - (size_t)m_header)
    ...
    for (i = 0; i < m_pTablesSize[TBL_AssemblyRef]; i++)
    {
        ...
        Dump_Printf(
            "AssemblyRef: %04x[%08x] %s (%d.%d.%d.%d)\n",
            i,
            ASMOFF(p),
            GetString(p->name),
            p->version.iMajorVersion,
            p->version.iMinorVersion,
            p->version.iBuildNumber,
            p->version.iRevisionNumber);
    }
    if (i)
        Dump_Printf("\n");
    ...
    for (i = 0; i < m_pTablesSize[TBL_TypeRef]; i++)
    {
        ...
        Dump_Printf(
            "TypeRef %04x[%08x] [Scope: %04x] %s %s\n",
            i,
            ASMOFF(p),
            p->scope,
            GetString(p->nameSpace),
            GetString(p->name));
    }
    if (i)
        Dump_Printf("\n");
    ...
    for (i = 0; i < m_pTablesSize[TBL_TypeDef]; i++)
    {
        ...
        Dump_Printf(
            "TypeDef %04x[%08x] [Extends: %04x] [Enclosed: %04x] [Flags: %08x] %s %s\n",
            i,
            ASMOFF(p),
            p->extends,
            p->enclosingType,
    ...
    for (i = 0; i < m_pTablesSize[TBL_FieldDef]; i++)
    {
        ...
        if ((NULL != strstr(tempFieldName, "<")) || (NULL != strstr(tempFieldName, ">")))
        {
            ...
            Dump_Printf("    // Something wrong with this field. Possibly its backing field is missing (mandatory for "
                        "nanoFramework).\n");
            Dump_Printf("    // ");
            ...
            strcpy_s(
                tempFieldName,
                ">>>>>>>>>>>>>>>>>>>>>>>>>>>>SOMETHING_WRONG_WITH_THIS_FIELD_POSSIBLY_MISSING_BACKING_FIELD");
        }
    ...
    for (i = 0; i < m_pTablesSize[TBL_Attributes]; i++)
    {
        ...
        Dump_Printf(
            "Attribute %04x[%08x] [%04x:%04x] %04x %04x\n",
            i,
            ASMOFF(p),
            p->ownerType,
            p->ownerIdx,
    ...
    for (i = 0; i < m_pTablesSize[TBL_Resources] - 1; i++)
    {
        ...
        Dump_Printf(
            "Resource %04x[%08x] [%08x:%1d] %04x %08x[%08x]\n",
            i,
            ASMOFF(p),
            p->id,
            p->kind,
            size,
            p->offset,
            ASMOFF(GetResourceData(p->offset)));
    }
    if (i)
        Dump_Printf("\n");
...
void CLR_RT_Assembly::Dump_MethodOwner(CLR_UINT32 idx)
{
    ...
    for (CLR_IDX i = 0; i < m_pTablesSize[TBL_TypeDef]; i++, td++)
    {
        if (idx >= td->methods_First &&
            idx < (CLR_UINT32)(td->methods_First + td->sMethods_Num + td->iMethods_Num + td->vMethods_Num))
        {
            Dump_Token(CLR_TkFromType(TBL_TypeDef, i));
            return;
        }
...
void CLR_RT_Assembly::Dump_Token(CLR_UINT32 tk)
{
    ...
    switch (CLR_TypeFromTk(tk))
    {
        ...
        {
            const CLR_RECORD_ASSEMBLYREF *p = GetAssemblyRef(idx);
            Dump_Printf("[%s]", GetString(p->name));
            break;
        }
        case TBL_TypeRef:
        {
            const CLR_RECORD_TYPEREF *p = GetTypeRef(idx);
            Dump_Printf("%s.%s", GetString(p->nameSpace), GetString(p->name));
            break;
        }
        case TBL_FieldRef:
        {
            const CLR_RECORD_FIELDREF *p = GetFieldRef(idx);
            Dump_Token(CLR_TkFromType(TBL_TypeRef, p->container));
            Dump_Printf("::%s", GetString(p->name));
            break;
        }
        case TBL_MethodRef:
        {
            const CLR_RECORD_METHODREF *p = GetMethodRef(idx);
            Dump_Token(CLR_TkFromType(TBL_TypeRef, p->container));
            Dump_Printf("::%s", GetString(p->name));
            break;
        }
        case TBL_TypeDef:
        {
            const CLR_RECORD_TYPEDEF *p = GetTypeDef(idx);
            if (p->enclosingType != CLR_EmptyIndex)
                Dump_Token(CLR_TkFromType(TBL_TypeDef, p->enclosingType));
            Dump_Printf("%s.%s", GetString(p->nameSpace), GetString(p->name));
            break;
        }
        case TBL_FieldDef:
        {
            const CLR_RECORD_FIELDDEF *p = GetFieldDef(idx);
            Dump_FieldOwner(idx);
            Dump_Printf("::%s", GetString(p->name));
            break;
        }
        case TBL_MethodDef:
        {
            const CLR_RECORD_METHODDEF *p = GetMethodDef(idx);
            Dump_MethodOwner(idx);
            Dump_Printf("::%s", GetString(p->name));
            break;
        }
        case TBL_Strings:
        {
            LPCSTR p = GetString(idx);
            Dump_Printf("'%s'", p);
            break;
        }
...
void CLR_RT_Assembly::Dump_Signature(const CLR_UINT8 *&p)
{
    ...
    switch (opt)
    {
        ...
        case DATATYPE_VALUETYPE:
            Dump_Printf("VALUETYPE ");
            Dump_SignatureToken(p);
            break;
        case DATATYPE_CLASS:
            Dump_Printf("CLASS ");
            Dump_SignatureToken(p);
...
static bool IncludeInStub(const CLR_RECORD_TYPEDEF *td)
{
    ...
    //
    if (td->flags & (CLR_RECORD_TYPEDEF::TD_Delegate | CLR_RECORD_TYPEDEF::TD_MulticastDelegate))
        return false;
...
static const CHAR c_Include_Interop_h[] = "#include <nanoCLR_Interop.h>\n"
                                          "#include <nanoCLR_Runtime.h>\n";

//--//

static const CHAR c_Type_Begin[] = "struct Library_%S_%s\n"
                                   "{\n";

static const CHAR c_Type_Field_Static[] = "    static const int FIELD_STATIC__%s = %d;\n";

static const CHAR c_Type_Field_Instance[] = "    static const int FIELD__%s = %d;\n";

static const CHAR c_Type_Method[] = "    NANOCLR_NATIVE_DECLARE(%s);\n";
...
static const CHAR c_Definition_Begin[] = "#include \"%S\"\n"
                                         "\n"
                                         "\n"
...
static const CHAR c_MethodStub[] = "    NANOCLR_HEADER();\n"
                                   "\n"
...
void CLR_RT_Assembly::BuildParametersList(CLR_PMETADATA pMetaData, CLR_RT_VectorOfManagedElements &elemPtrArray)
{
    ...
    for (CLR_UINT32 curElem = 0; curElem < elemCount; curElem++)
    {
        ...
        switch (opt)
        {
            ...
            case DATATYPE_BYREF:
                pNewElement = new CLR_RT_ManagedElementTypeByRef(CLR_UncompressElementType(pMetaData));
                break;
            case DATATYPE_SZARRAY:
                pNewElement = new CLR_RT_ManagedElementTypeArray(CLR_UncompressElementType(pMetaData));
                break;
            ...
            default:
                pNewElement = new CLR_RT_ManagedElementType(opt);
        }
...
{
    CHAR buffer[256];
    CLR_PMETADATA p = GetSignature(md->sig);
    p++; // Skip Calling convention.
    CLR_UINT32 len = *p++ + 1;
        ...
        while (fContinue)
        {
            ...
            switch (opt)
            {
                ...
                case DATATYPE_CLASS:
                {
                    ...
                    {
                        case TBL_TypeDef:
                            BuildClassName(GetTypeDef(CLR_DataFromTk(tk)), cls, true);
                            break;
                        case TBL_TypeRef:
                            BuildClassName(GetTypeRef(CLR_DataFromTk(tk)), cls, true);
                            break;
                        case TBL_TypeSpec:
                            sprintf_s(buffer, ARRAYSIZE(buffer), "_%08X", tk);
                            cls = buffer;
                            break;
                    }
                    ...
                    while (true)
                    {
                        std::string::size_type pos = cls.find('_');
                        if (pos == cls.npos)
                            break;
    ...
    if (num++)
    {
        sprintf_s(buffer, ARRAYSIZE(buffer), "_%d", num);
        name += buffer;
    }
    mapMethods[name] = num;
}
...
{
    for (string::iterator strp_Iter = strName.begin(); strp_Iter != strName.end(); strp_Iter++)
    {
        if (*strp_Iter == '.')
        {
...
static string BuildCalltoNativeStub(
    bool bStaticMember,
...
static string BuildDeclarationOfNativeStub(
    string strMethodName,
    string strAssemblyName,
    CLR_RT_VectorOfManagedElements &elemPtrArray,
    bool bStaticMethod)
{
    ...
    {
        // For each parameter add string like " INT8 param0,"
        strCall += elemPtrArray[i]->GetVariableDecl() + " param" +
                   CLR_RT_ManagedElementType::GetParamIndexAsText(i - 1) + ", ";
    }
...
void CLR_RT_Assembly::GenerateSkeletonStubFieldsDef(
    ...
    string strNameSpace)
{
    if (pClsType->iFields_Num) // If fields are present.
    {
        ...
        for (int curFld = 0; curFld < pClsType->iFields_Num; curFld++, fd++)
        {
            ...
            {
                // something very wrong with field name!!
                string tempMessage = strIndent + "// Something wrong with this field. Possibly its backing field is "
                                                 "missing (mandatory for nanoFramework).\n";
            ...
            string strFieldDef = strIndent + "static " + elemType.GetStandardTypeName() + "& Get_" + tempFieldName +
                                 "( CLR_RT_HeapBlock* pMngObj )    ";
            // Now generates the body of the function.
            strFieldDef += "{ return Interop_Marshal_GetField_" + elemType.GetNativeType() + "( pMngObj, " +
                           strNameSpace + "::FIELD__" + tempFieldName + " ); }\n\n";
    ...
    */
    ...
    for (int i = 0; i < m_pTablesSize[TBL_TypeDef]; i++, td++)
    {
        ...
        if (totMethods && AreInternalMethodsPresent(td))
        {
            ...
            FILE *pFileMshlSrc = NULL;
            Dump_SetDevice(pFileMshlSrc, rgFiles);
            ...
            FILE *pFileStubHead = NULL;
            Dump_SetDevice(pFileStubHead, rgFiles);
            ...
            FILE *pFileStubSrc = NULL;
            Dump_SetDevice(pFileStubSrc, rgFiles);
            // Adds file to the CMake module file.
            Dump_Printf(pFileCMakeModule, "    %S\n", GetFNameWithExtFromFilePath(rgFiles).c_str());
            ...
            Dump_Printf(
                pFileStubHead,
                (strIndent + "// Helper Functions to access fields of managed object\n").c_str());
            GenerateSkeletonStubFieldsDef(
                td,
                pFileStubHead,
                strIndent,
                "Library_" + string(szCharName) + "_" + cls_name);
            ...
            Dump_Printf(
                pFileStubHead,
                (strIndent + "// Declaration of stubs. These functions are implemented by Interop code developers\n")
                    .c_str());
            for (int j = 0; j < totMethods; j++, md++) // Iterates over all methods in class.
            {
                if (IncludeInStub(md)) // Test if it is internal method.
                {
                    ...
                    // If function is not static - we need to retrieve addresses ( in form of references ) of all member
                    // variables.
                    if (!bStaticMethod)
                    ...
                    strFuncImpl +=
                        "        " +
                        BuildCalltoNativeStub(bStaticMethod, strNativeStubName.c_str(), strMngClassName, elemPtrArray) +
                        "\n";

                    //****************** Declaration and definition for the stub method. ******************
                    // Example: "INT32 TestBasicTypes::Add_Values_Array_double( param0, arraySize0, hr );"
                    string strNativeStub =
                        BuildDeclarationOfNativeStub(strNativeStubName.c_str(), string(), elemPtrArray, bStaticMethod);
                    ...
                    Dump_Printf(
                        pFileStubSrc,
                        BuildDeclarationOfNativeStub(
                            strNativeStubName.c_str(),
                            strMngClassName,
                            elemPtrArray,
                            bStaticMethod)
                            .c_str());
                    ...
                    {
                        strFuncImpl +=
                            "        SetResult_" + elemPtrArray[0]->GetNativeType() + "( stack, retVal );\n\n";
                    }
...
void CLR_RT_Assembly::GenerateSkeletonFromComplientNames(LPCWSTR szFilePath, LPCWSTR szProjectName)
{
    ...
    swprintf(
        rgFiles,
        ARRAYSIZE(rgFiles),
        L"%sFindINTEROP-%S.cmake",
        GetDirPathFromFilePath(szFilePath).c_str(),
        m_szName);
    FILE *pFileCMakeModule = NULL;
    Dump_SetDevice(pFileCMakeModule, rgFiles);
    ...
    //
    {
        swprintf(rgFiles, ARRAYSIZE(rgFiles), L"%s.h", szFilePath);
        Dump_SetDevice(rgFiles);
            ...
            if (IncludeInStub(td))
            {
                ...
                if (td->sFields_Num)
                {
                    ...
                    for (int j = 0; j < td->sFields_Num; j++, fd++)
                    {
                        if (fSeen == false)
                        {
                            Dump_Printf(c_Type_Begin, szName, cls_name.c_str());
                            fSeen = true;
                        }
                        ...
                        {
                            // something very wrong with field name!!
                            Dump_Printf("    // Something wrong with this field. Possibly its backing field is missing "
                                        "(mandatory for nanoFramework).\n");
                            Dump_Printf("    // ");
                            Dump_Printf(tempFieldName);
                            Dump_Printf("\n");
                ...
                if (td->iFields_Num)
                {
                    ...
                    for (int j = 0; j < td->iFields_Num; j++, fd++)
                    {
                        ...
                        Dump_Printf(
                            c_Type_Field_Instance,
                            &tempFieldName,
                            m_pCrossReference_FieldDef[j + td->iFields_First].m_offset);
                    }
    ...
    //
    {
        swprintf(rgFiles, ARRAYSIZE(rgFiles), L"%s.cpp", szFilePath);
        Dump_SetDevice(rgFiles);
...
void CLR_RT_Assembly::BuildMethodName_NoInterop(
...
void CLR_RT_Assembly::GenerateSkeleton_NoInterop(LPCWSTR szFileName, LPCWSTR szProjectName)
{
    ...
    for (std::basic_string<char>::iterator strp_Iter = strAssemblyIDName.begin(); strp_Iter != strAssemblyIDName.end();
         strp_Iter++)
    {
        if (*strp_Iter == '.')
        {
    ...
    //
    {
        swprintf(rgFiles, ARRAYSIZE(rgFiles), L"%s.h", szFileName);
        Dump_SetDevice(rgFiles);
    ...
    //
    {
        swprintf(rgFiles, ARRAYSIZE(rgFiles), L"%s.cpp", szFileName);
        Dump_SetDevice(rgFiles);
        ...
        Dump_Printf(c_Definition_End, strAssemblyIDName.c_str(), m_szName, m_header->nativeMethodsChecksum, 0, 0, 0, 0);
            ...
            if (totMethods && IncludeInStub(td))
            {
                ...
                for (int j = 0; j < totMethods; j++, md++)
                {
                    if (IncludeInStub(md))
                    {
                        if (fFirst)
                        {
                            swprintf(rgFiles, ARRAYSIZE(rgFiles), L"%s_%S.cpp", szFileName, cls_name.c_str());
                            Dump_SetDevice(rgFiles);
